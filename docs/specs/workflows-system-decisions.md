...
- **Envelope schema is generated, never hand-written:** system-owned control fields at
  ...
  with the same schema attached. There is no provider-level suspension mechanic.
  No fourth `failed` status: check failures belong to deterministic check phases +
  the diagnosis phase, not self-assessment; "can't proceed" is `stuck`.
...
2. **Value keywords are decorative on Codex** (`minimum`, `maxLength`, etc. are
...
3. **"Valid-or-explicit-error" is FALSE as the spec states it, on both
   ...
   the fold-in.
4. **Wiring:** on Claude, `--json-schema '<inline JSON>'` (inline string, NOT a
   ...
   On Codex, `outputSchema` (camelCase) in `turn/start` params; **must be re-sent
...
5. **Takeover/finalize fits cleanly:** Codex free-form turns are naturally
   ...
   encouraged tool, free-form turns behave normally, which is acceptable.
...
- **Jira integration (user ruling, 2026-07-11): do NOT depend on the user's
  ...
  target the official Atlassian CLI (`acli`) and/or the Atlassian remote MCP
  server for agent-driven phases (grantable via profile MCP config). `acli` is
  verified adequate: GA for Jira, `workitem search --jql ... --json --fields ...`
  covers polling with an `updated >= <cursor>` watermark; workitem
  view/edit/comment/transition cover report-back. `gh`/`glab` stay as-is for
  GitHub/GitLab. Building our own Jira CLI is a fallback only if `acli` proves
  inadequate for the narrow poll/report surface; not planned.
...
- **Storage:** `<app-config>/projects/<slug>/profile.yaml`, under the same
  not-in-repo rule and the same loader/validator infrastructure as workflows
  (D1); agent-editable via the studio. Project *identity* stays the DB
  `projects` row; `projects` gains a stable unique `slug` column.
- **Contents:** default base branch; check commands (name → argv); resource
  ...
  template, e.g. `acli`, `gh`, `glab`; never a hardcoded tool); reliability
  defaults (watchdog T, default per-item budget); disposition policy (`manual` |
  `auto-pr` | `auto-merge`, default `manual` per D3.3); secret references (v1:
  `env` and `file` sources; OS keychain later); MCP servers grantable to phases.
...
- `work_item_phases`, one row per phase *attempt*: item_id, phase_id, attempt,
  thread_id (denormalized, no FK, since records outlive threads, usage_ledger
...
- `work_item_effects`, the A5 surface-and-skip ledger: (item, phase, tool,
...
- Budget: checked at phase boundaries via `usage_ledger` sum over `work_item_id`
  (includes takeover/finalize turns, which run in phase threads).
...
- **Workflow settings live in their own settings area** (rarely changed):
  concurrency, per-project capacities, notification prefs. It is a UI over the
...
- **Phase grants become token scopes:** §5's "tools granted to a phase" maps to
  ...
  run context, set at phase entry) authorize, which is cleaner than MCP
  allowlists. Effect recording (A5 surface-and-skip) happens server-side keyed
  by the token's (run, phase) context.
...
- **Cleanup:** `cleanup: auto | manual` (default manual). `auto` = worktree
  ...
  `auto` always allowed for derived-read-only workflows (no worktree, so nothing
  to clean; the setting is inert); for writing workflows `auto` only takes
  effect after a successful disposition (auto-pr / auto-merge landed), so an
...
history/sidebar** like phase threads, under their own mode, reachable from the
...
- **Worktree setup:** profile gains `worktree_setup: {copy: [...], run: [...]}`.
  Files are copied from the main workspace (e.g. `.env`) and commands run (e.g.
...
- **D4.5 amendment: enqueue-from-chat is user-directed, not autonomous.**
  ...
  (normal chat + triage) via the existing per-provider MCP plumbing. It is
...
- **D11 amendment: PR follow-ups become threads.** The bespoke
  ...
  PR**, which opens/reuses that thread seeded with PR context to review and
...
- **D18. Call phases** (supersedes "sub-workflow — post-v1"). A phase may
  invoke a workflow by **static id** (never a variable, since the dry-run
  validates the whole call graph). The child is a **real run**, with its own
  item, phases, counters, and record, linked into the parent's tree. **Workspace
...
- **D11 amendment: the overlay replaces the pane + sidebar section.** The
  ...
  detail: `docs/specs/workflows-system-ui/UI-SPEC.md` rev 2.
...
- **D25 (RESOLVED 2026-07-25). Deleting a project cleans up after itself; it
  ...
  - `App.ProjectDeletionPreview` (`app_project_delete_workflow.go`, LocalOnly)
    ...
    (`workflowTreeLoss`), because which checkouts a run tree owns is one
    question with one answer, but not the row type: a discard row reports
    unmerged commits as a loss, and under cleanup those commits are not lost,
    so reporting them would be actively misleading.
...
- **D32. Every affordance that started a NEW chat thread from a workflow
  ...
  **A better primitive may replace this later.** The removal is clean (no
  dead code, no commented-out buttons, no orphaned bindings) so that
...
- **D37. The campaign starter is two workflows: a spine that calls itself
  ...
  **`checkpoint-every` is a human gate the planner arms.** A gate
  predicate cannot do arithmetic, since the language is comparisons,
...
- **D38. A surface that reports a parked run must name the command that
  ...
  **The verb needs its arguments, so the arguments became readable.**
  ...
  of `run retry-unit`, resolved on the single-run read only, since a list
...
- **D41. A park is settleable from the CLI, under a grant an author hands out
  ...
  **Amendment (same day): `run resolve` settles `human:` routes only. A
  ...
  (resolved app-side from the gate trace, best-effort, since an unreadable
  trace names both verbs rather than guessing); the composer repair map carries
...
- **D49. A fan-out unit's branch name keys on the full provisioning
  ...
  …). Lane retirement removes checkouts, never branches (deliberate, since the
...
- **A Codex error is information; the completion is the verdict.**
  ...
  empty-payload completion, never from the error notification. That is the
...
- **D72. An undecodable guidance slot heals instead of bricking.** The
  ...
  never truncated, since that line is the only surviving copy), the column is
...
- **D70. `retries-exhausted` joins the continuable parks.** User-reported:
  ...
  - `stalled` deliberately not included: the watchdog gave up on a session
    ...
    session, which is its own future decision. `stuck` untouched: an
...
- **D69. One spend fold serves enforcement, display, and the prompt.** The
  ...
  `checkBudget` is the one place it becomes an error, since returning an
  error from the read took `run status` and the binding away over exactly
  the fact the operator needed, and the old in-source refusal parked
  ...
  predicate, and the anti-change list holds). Accepted cost, stated in the
...
- **D65. Reserved `call-depth` read.** The engine's `CallDepth` counter,
  ...
  name, and the engine's answer must win. The campaign starter keeps its
...
- **D67. Workflow outputs can be `optional:`, and required ones must be
  ...
  `childOutputEnvelope` fails a missing declared output, so the campaign
...
- **D63. Goal chain + `non_goals:` in every element's prompt.** The live
  ...
  with `regressed` being the state a 40-wave port actually produces) forwarded
  through the self-call args like the wave number, dry-run-checked in the
  campaign starter.

- **D64. With `accounts_for_units:`, the merge join must account for every
...
- **D60. `session: continue | fresh` on loop routes.** The measured
  ...
  tool/call/fan-out phases (no single session to continue, and the finding
...
- **D57. Campaign memory: one append-only log per root run, app-owned,
  ...
  infrastructure nobody used, since this is one NDJSON file; a
  human-graduation gate that made notes a write-only log, since promotion
  is automatic, per the user's autonomy ruling). Mechanics:
  ...
  - **Storage**: `<configDir>/workflow-memory/<root-run-id>/notes.ndjson`,
    ...
    lazily on first note. Outside the repo and the worktree, so no cleanup
    debris on the deliverable branch (orc's C4 cautionary). `wave` is the
    writing run's existing `CallDepth`, with no parallel counter. Appends are
    single-write; a torn final line costs exactly one note (the next
    append heals the unterminated tail, and the first build had a
  ...
  - **Two write channels, the narrative precedent exactly**: `memory add`
    ...
    (row-level authority, no declared grant, because writing memory is part of
    ...
    the narrative seam in both drivers, and the tool driver lifts it too, so
  ...
  - **Injection is the promotion.** Every element prompt carries the path
    ...
    from the file, with no rendered-brief cache to go stale. All text through
    `untrustedtext`.
  - **Lifecycle**: survives completion AND discard (discard deletes no run
    records, verified and matched), deleted by project deletion alongside
...
- **D54. `notify:`, a route decoration that wakes without parking.** The
  ...
  routes refuse it statically (the park already wakes, and a decoration there
  ...
  not invented, since an OS notification per wave is the interruption budget
...
- **D53. Every engine-side park persists the engine's own diagnosis, on an
  ...
  - **A park always has a row to rest on.** A phase known but never
    ...
    whose workflow never resolved has no phase to rest on, so the cause goes
...
- **D53a. The engine has a log stream.** `<configDir>/logs/engine-YYYY-MM-DD.ndjson`,
  ...
  stderr. It is a maintainer surface, and the operator-facing record stays
...
- **D52. The run's read surface is the CLI's obligation, not the
  ...
  [--unit]`, which prints the narrative through the same resolver the wake
  uses (a path is never derived twice); `seeds` on `run status --json`. Both
...
- **D50. `--refresh-def` re-reads a parked run's definition at a fresh phase
  ...
  re-stamped by a resume refresh, since the wall-clock budget measures
  against it. No UI affordance yet; the frontend passes `false`, and
  exposing it is a product decision.
