...
> specified in `docs/specs/workflows-system-ui/UI-SPEC.md` (rev-2 overlay rewrite
...
> with **failed → running** on a rerun
...
- **running**: actively being driven through its workflow. An item is running
  ...
  item out of it.
- **needs-human** means paused for a person: a gate, a question, genuinely stuck, a
...
(workflow input, phase input, fan-out `as:`, and phase **id**, since a phase
named `call-depth` would have its output object silently shadowed), and bound after
...
- **fan-out/fan-in**: N parallel units → a join that consolidates their
  ...
  (agent), a `command:` (tool), or a `call:` (another workflow, §3a). Any
  ...
  **own driver/provider/model/effort/access** (mixed
  ...
  - **unit failure policy:** a unit that exhausts its §12 retries stops the
    ...
    (their work is durable, and interrupting wastes it); then the phase parks
...
- **Absence crosses the edge as absence.** Arguments evaluate where the
  ...
  **omitted** when the child input it seeds is declared `optional:` (the
  child sees the same absence a direct start without that seed would give
  it), and refuses (`wiring-error`, message naming `arg (ref)`) only when the
...
absent optional call arg crosses a call edge (D45), never a failure. That is
the repair for the incident where a campaign died at the moment it completed,
...
agent element's prompt (units and the join included, since the join decides
what ships) opens with a bounded **goal-chain block**: the goals from the
...
fragments** (string/number/boolean/enum/array/object, with no custom type system);
...
cheaper exit. The pattern for severity-aware exhaustion is to have the
deciding phase emit a routable output (e.g. `worst-severity`), then:
...
cancel. The run stays parked exactly as it was (D47; converting the park
...
authorize only the subcommands the workflow granted** that phase, and "tools
...
CLI obligation (D52):** everything an operator needs to decide a park is one
verb away: worktree and branch, seeds, children, per-attempt provenance and
envelope output digests (`run inspect`, with `--phase` drill-down), the
engine's park cause (D53, bounded on attempt lines, whole under `--phase`),
and the narrative prose (`run narrative`). That matters because the live
...
- **child runs** (§3a) never bind and never notify; they surface through the
  ...
  the parked descendant (its run id, workflow, typed reason, and parked
  phase). A subtree blocked on a question is never invisible just because the run
...
- **start a run**, for decomposition: spinning off independent work that is *not*
...
- **resolve**, to decide the parks a run this phase started rests on:
...
**redelivers rather than loses**, since a lost instruction is the worse failure),
...
prompts, the same ownership truth as `run amend`). Entries are system-stamped
...
- **Complete**: the system runs one **finalize turn that re-attaches the
  ...
  instead fill the envelope's fields by hand, since it is structured, with no
  model turn.)
...
Interrupting leaves the worktree **mid-edit** (partial state). That is fine:
you took over to fix things, which is why the natural finishes are Complete or discard
...
- **Take over** by driving it yourself, exactly as above: open the phase's
...
and conflict-resolution loop (§9), a recursive batch driver (§3a). Authoring
is therefore "tweak an example," not "design from scratch." Content, not
engine.
...
sandboxed in the provider's restricted mode (no file writes, no destructive
commands); `write` runs with full access **in its own isolated workspace**. A
...
worktree at terminal state once §3 artifacts are captured, and for writing
...
**Two join patterns, and the author picks by what the join does:**
...
**Parallel-writer fan-out vs decomposition** look alike but differ in
*what survives*:
...
each with kind / when / note). It is also the **recovery journal** (§12).

### Notifications

- A **native OS notification** fires on **`needs-human`** and **`failed`**
  only (never on `done` / `running`, which are the passive badge), carrying
...
**polling through the user's already-authenticated CLIs**, which are
**profile-bound commands** (§8: `gh`, `glab`, `acli` for Jira) and never a tool
name hardcoded in the engine. They are **not** reached by inbound webhooks.
A poll is just a **cron automation
...
latency), not instantly, which is fine for coding automations. **Webhook ingress**
...
- **Per-item budget.** One **optional** ceiling per item (**tokens / $** if
  ...
  **The spend is truthful across providers, and one number serves every
  ...
  discovering the ceiling by being parked at it. It is a prompt surface only,
