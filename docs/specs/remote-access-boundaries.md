...
| 9 | Repeated guessing against `/auth/token` or the ticket endpoint | Secrets are 256 random bits, so no achievable rate makes guessing plausible; what is bounded is WORK, by a per-peer token bucket on the auth family (`internal/transport/ratelimit.go`), shared across listeners. There is no token- or account-keyed counter. |
| 10 | A web page uses DNS rebinding to address the loopback listener | Strict Host allow-list and Origin checks on `/ws` and the auth endpoints, rebound Hosts rejected. `Sec-Fetch-Site` is not consulted: the Host guard is the mechanism, and `Sec-Fetch-Site` would be a hint from the same untrusted agent. |
