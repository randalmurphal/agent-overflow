...
| **soak rig** | A second, fully isolated copy of the real desktop app left running for hours with harness-grade mocking, to reproduce renderer hangs. Nothing asserts; ask `make soak-check` (or `ao-harness health`) later. Two shells over one `--soak` backend: the Windows launcher (`docs/architecture/soak-rig.md`) and the native window (`make soak-window`). |
