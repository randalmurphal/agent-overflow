...
| **the isolation pins** | The App fields `TestMockedBootModesShareOneIsolationHelper` (`main_soak_test.go`) names: the set every mocked boot mode must take together, assigned only in `internal/app/bootstrap.go`. A partial set is the shape that burned a real login and later wrote a mock fixture into a real repository. |
