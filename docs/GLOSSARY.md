...
| **item** | (a) work item: a workflow run (`workflows-system.md` §2). (b) timeline item: a `store.Item` row with `item_index` ([conversation history](architecture/invariants.md#conversation-history)). Both docs say "the item" with no qualifier. |
...
| **lease** | (a) scroll lease: depth-counted auto-scroll suspension (`docs/architecture/scroll-contracts.md`). (b) input lease: the take-control arbitration token on a Claude-TUI terminal (`docs/architecture/claude-tui-provider.md`). (c) expansion lease: an activity-run row's keep-me-mounted claim while expanded (`activity-runs.md`). (d) lifecycle lease: a connection's statement that its whole CLIENT is paused by the platform, which the backend answers with withheld highlight seeds and merged transcript deltas (`internal/transport/lease.go`). |
...
| **watch** | (a) transport watch: the set of thread ids a connection declares it is looking at, which narrows the entity-filtered event channels to those threads ([watched entities](architecture/transport.md#watched-entities-and-paused-clients)). Its frontend half is the *watched set* in `stores/watchedThreads.ts`, composed from surfaces that EXIST and never from what is visible. (b) git watch: a `gitwatch.Subscription`, one per cwd, feeding `git:status`. (c) dev watcher: the Wails3 fsnotify watch over the repo (`build/config.yml`). |
...
| **stash** | `pending_background_task_terminals`: a queued completion held until the agent observes it, drained by `task_notification` as described in [task lifecycle](architecture/turn-lifecycle.md#2-task-lifecycle-claude-only). The notification is a timing trigger, never a status source. |
| **turn** | The unit `Stop` interrupts. Turn activity is wire-pushed, never derived from items; `turn_index` is monotonic per thread ([turn lifecycle](architecture/turn-lifecycle.md#3-turn-lifecycle)). |
...
| **the isolation pins** | `providerBinaryOverride`, `credentialHomeOverride`, `fileKeychainOverride`, `backgroundFetchDisabled`, `isolatedWorkspaceRoot`: the set every mocked boot mode must take together, assigned only in `internal/app/bootstrap.go`. A partial set is the shape that burned a real login and later wrote a mock fixture into a real repository. |
