...
| Term | Definition |
|---|---|
| **harness** | `--harness` boot mode: real backend + real SPA, headless, isolated data dir, both provider binaries pointed at `ao-mockprovider` (`docs/architecture/agent-harness.md`). |
| **soak rig** | A second, fully isolated copy of the real desktop app left running for hours with harness-grade mocking, to reproduce renderer hangs. Nothing asserts; ask `make soak-check` later (`docs/architecture/soak-rig.md`). |
