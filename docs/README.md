...
| [`agent-thread-tools-plan.md`](architecture/agent-thread-tools-plan.md) | Build order, data model, ownership and edge cases for the `ao-thread-tools` spec; amendments the plan makes to the spec are listed at its top. Not started. |
...
| [`agent-thread-tools.md`](specs/agent-thread-tools.md) | The `ao-thread-tools` MCP server (thirteen tools: search, show, item, options, spawn, send, ask, reply, status, cancel, update, group, remind) and `/side-chat`, locally and across paired computers. Settled 2026-09-19; not implemented. |
