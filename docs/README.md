...
| [`subagent-scope-windows-plan.md`](architecture/subagent-scope-windows-plan.md) | Subagent pane and tools-only tray windows using the main timeline’s page, run-member and recovery infrastructure. |
...
| [`app-update.md`](specs/app-update.md) | In-app updates on macOS, Linux and Windows: database snapshot, trial boot, commit or rollback, recovery after a crash, and no live migration. Implemented except serve's trial budget. |
