...
| `thread_fork_lineage`, `thread_fork_hidden`, `thread_fork_copied` | Pointer-fork reads ([sqlite-store.md](sqlite-store.md#pointer-forks)). `thread_fork_lineage` has one row per ancestor a fork reads through, keyed by `(thread_id, depth)`, with the ancestor and the cut for that level. `thread_fork_hidden` lists the inherited ids a fork does not show: rows it copied, reverted or deleted, rows fork creation excludes (live background launches, what an item cut leaves out, and what hangs off either), and rows an ancestor added or moved below the cut later. Both cascade with the fork's thread row. `thread_fork_copied` lists the copies of an unfinished `MaterializeForkHistory` that no write of the fork has made its own, each with `source_id`, the ancestor that held the row (`idx_thread_fork_copied_source`); an ancestor's delete rolls back the copies of the rows it and the threads beyond it held. Each row cascades with its item, and the materialization's last transaction clears them. |
...
thread whose delete has begun (`threads.deleting`), so listings, search,
transfers and fork admission never reach a partly deleted thread.
...
| Pointer forks | `forkTriggersSQL`: hide rows an ancestor inserts or moves below a fork's cut, keep a fork's own rows at or after its cut, advance the stamps of forks that show an ancestor row updated in place, and refuse to delete a thread forks still read through ([sqlite-store.md](sqlite-store.md#triggers-and-stamps)). |
