...
| `work_items` | Durable workflow run records (migration v23, extended v25): denormalized project/workflow identity, goal and seeds, frozen workflow snapshot, queue order, lifecycle state + typed reason, step mode, worktree/branch metadata, budget, source attribution, optional `triage_thread_id`, and timestamps. No project/thread FK: history outlives either row. |
