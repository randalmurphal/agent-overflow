...
| `work_items` | Durable workflow run records (migration v26, extended v28/v29, rebuilt v33): denormalized project/workflow identity, goal and seeds, frozen workflow snapshot, lifecycle state + typed reason, step mode, worktree/branch metadata, budget, source attribution, optional `triage_thread_id`, JSON disposition receipt + digest, and timestamps. No project/thread FK: history outlives either row. v33 removed the queue: `sort_position` is gone and `state` no longer admits `'queued'` (surviving queued rows became `needs-human` / `interrupted`). |
