...
| `subagent_aggregate_backfill` | Migration v121's deferred phase: the threads whose subagent anchors predate the write-time cards. The app stamps them in paced batches and deletes a thread's row with its last unstamped anchor; until then reads walk that thread's unstamped anchors. An empty table is the applied phase. |
...
| Subagent aggregate indexes | `idx_items_subagent_resume_prompt` is the history triggers' round probe: the latest resume prompt under an anchor at or before a written row. `idx_items_subagent_aggregate_dirty` finds a thread's dirty anchors by key for `RecomputeSubagentAggregates`. `idx_items_running_nested_fg_tool_calls` holds the nested foreground calls in flight, which `ListLiveBackgroundTasks` walks up to their background launch instead of walking every launch's subtree down. |
...
| Family | Purpose |
|---|---|
| History revision | Three `items` triggers maintain `threads.history_rev`, `threads.history_epoch`, and the per-row `items.rev` stamp on every row whose read result the write changed: the row, its completion sibling, and the anchors decorated from its parent chain (`stampedRowIDsSQL`: a recursive CTE walks the chain by primary key; the carrier leg probes the partial expression index `idx_items_transcript_root`). Between the thread bump and the row stamp they keep each subagent anchor's card and `subagentAggregateState` on the anchor's meta, per nesting level of the written row (`subagent_aggregate_stamps.go`). `rev` is written only here; the update trigger's `WHEN OLD.rev IS NEW.rev` guard keeps the stamping write from re-bumping the thread. |
