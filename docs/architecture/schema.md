...
| `work_items` | Durable workflow run records (migration v23): denormalized project/workflow identity, goal and seeds, frozen workflow snapshot, queue order, lifecycle state + typed reason, step mode, worktree/branch metadata, budget, source attribution, and timestamps. No project/thread FK: history outlives either row. |
