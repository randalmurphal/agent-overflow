...
| Family | Purpose |
|---|---|
| History revision | Three `items` triggers maintain `threads.history_rev`, `threads.history_epoch`, and the per-row `items.rev` stamp on every row whose read result the write changed: the row, its completion sibling, and the anchors decorated from its parent chain (`stampedRowIDsSQL`: a recursive CTE walks the chain by primary key; the carrier leg probes the partial expression index `idx_items_transcript_root` by root id). Between the thread bump and the row stamp they mark dirty the `subagent_aggregates` rows a write may have changed, unless the write claimed its anchor (`subagentClaimRev`) and the store applies its effect with keyed writes (`subagent_aggregate_writes.go`), and they strip card keys from a written meta (`subagent_aggregate_stamps.go`). The row stamp writes `rev`; the only other value any write stores there is a claimed write's `subagentClaimRev`, which the stamp replaces in the same statement. The update trigger fires on every column but `rev`, so a stamp does not re-bump the thread, and its `WHEN OLD.rev IS NEW.rev` guard, which lets a claimed write through by name, excludes the served-key strip. |
