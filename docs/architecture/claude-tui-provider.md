...
| `PostToolUseFailure` | tool **failure** completion; *no* `tool_response`; fires *instead of* PostToolUse, so register both or failed tools never complete | required |
...
same `UserInputResponse` path the headless provider uses, with no new frontend
branch.
...
Point Claude at the user's actual config directory, which inherits native trust,
...
- **Terminal vs stall.** A completion carries `<status>` (`completed` / `failed`
  ...
  body is skipped because the task is still running. `NormalizeTaskTerminalStatus` is
  the shared gate.
...
It is NOT all one user-facing `EventError`. It splits by whether
