...
- `mu` — 7 clusters before this wave, and that was the defect stage 1
...
- `workspaceFiles` (6), `terminals` (6), `replay` (6),
  ...
  `providerCredentials` (4) — all injected collaborators, all fine as
  `Deps` fields.
...
`backgroundFetchCancel` → `backgroundFetchMu` (moved as a pair — they
are set and cleared in one critical section). Both new locks are
...
- **`//wails:id` pins an ID explicitly** (Wails
  ...
  already used for `Telemetry`, `Shutdown`, the `ao` token minter, etc.
