...
| Card flush inside an item write whose card no running agent covers (`writeItems`, `subagentCardLiveSQL`) | none of its own: the item write already bumped; the keyed `subagent_aggregates` writes' trigger stamps the anchor and completion siblings | rev, once per write |
