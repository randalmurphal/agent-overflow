...
The generated CLI reference is [docs/reference/ao-harness.md](../reference/ao-harness.md). It lists the descriptor tree and points to `rpc --list` for the instance-specific method catalog. The reusable half is `internal/harnessclient`: bootstrap discovery
...
private cgroup v2 (`memory.max`, swap disabled, OOM group kill), and macOS
inherits `RLIMIT_DATA` through a same-PID exec wrapper because macOS has no
aggregate process-tree primitive. On Windows, `ao-harness up --window` and
...
a 100ms detached watchdog and visible evidence. A failed macOS limit installation aborts the launch. The
ordinary `bench` command deliberately operates on a selected borrowed
