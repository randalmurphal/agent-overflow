...
1. **Perceived performance first: high refresh.** Target displays run
   ...
   **~4–6ms including style/layout/paint**, not the 16ms folklore
   ...
   - **Hot-thread switch is a measured scenario**: with a priors hit,
     a revisited thread must paint once at final geometry, with zero
...
- **The engine never writes `scrollTop`. Ever.** It owns geometry
  ...
  **compensation observation** into the scroll controller. That is the same
...
- **Coordinates stay top-anchored prefix sums** (inventories,
  ...
  - *Reading position*: when not pinned, an above-viewport size delta
    ...
    the reading anchor. Those are the outcomes currently pinned by
...
desync, so a decline simply means "the pin write this frame already
covers it").
...
- Props: `data`, `getKey`, `estimateSize` (56px fallback under
  ...
  today), `scrollRef` (external scroller: the engine never owns the
...
(destroyed-flag on the mount tick, dispose cancels everything, with
...
- **Deleted**: patch tripwires, marking seam test, shift-cache
  tripwire, cache-replay tests. Each is replaced by a bespoke-side
