...
2. **Intent is unobservable, so it is inferred.** Tags, token FIFOs, 1ms
   deferred checks, resize-correlation budgets, version counters are all
...
5. **Bugs live in the seams between ~10 interacting state machines** (spring,
   ...
   They are each individually defensible, each with cross-couplings like the
...
orchestration, paging, and window-anchor transactions, which are
component-scoped choreography, not controller business.
...
- **Pane/consumer surface (~7):** `isAtBottom` read, `lease()`
  (depth-counted; release-at-depth-0 re-pins synchronously, a hard contract),
...
| Scroll-token duplicate budget (4). **MOVED to Stage 2** (one write/frame makes the multi-token case structural, not tuned) | Streaming capture of token consumption | Plausibly tuned during churn era; 1-2 may suffice. It is fully moot under one-write-per-frame |
...
controller), `trace.ts` (shared dev-trace helper), and `time.ts` (`nowMs`).
The controller file is `index.svelte.ts` imported as
...
- A bespoke engine is ~9 subsystems, only two genuinely hard: the
  bottom-anchored one-resolve anchor policy (which *eliminates* virtua's
  entire jump/frozen-range feedback protocol and most of its shouldKeep
  table), and DPR-epsilon equality (already learned via `db77ba0d`). Intent
...
module-level controller state exists, leaving cross-pane interference
unexplored; (3) ChannelView has zero scroll tests despite the doc claiming
the same contracts; (4) escape gestures never run against trusted input in
a real engine (`@vitest/browser` userEvent can close this later).

**Cost note:** the controller suite is 27.2s, the dominant cost of the
whole frontend unit gate, mostly real-timer sleeps around spring warm-ups.
Stage 2's pure reducer converts that choreography into fast exhaustive unit
tests; expect the suite to shrink in wall time while growing in coverage.
