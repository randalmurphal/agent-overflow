...
None in `frontend/`. Archived-thread restore remains backend-blocked, and the UI now surfaces that limitation explicitly instead of implying a working restore flow.
...
| R21 | Integration Wiring | 5 findings: (1) BranchToolbar and GitActionsControl loaded git state only on mount, so switching threads left stale branch/action state from the previous thread; both now reload from `pane.threadId` reactively. (2) Sidebar cached `defaultProvider` only once, so the new-thread form ignored later settings loads/changes; opening or resetting the form now re-seeds from current settings. (3) Worktree create/remove flows changed backend thread metadata but left frontend thread state stale; frontend now refetches the updated thread via `GetThread`, updates pane/sidebar state, and refreshes settings after thread creation so recent workspaces stay current. (4) ProviderSettings only fetched provider health/models on mount, so binary-path edits never refreshed status/model lists; it now reloads reactively from settings path changes. (5) ProviderStatusBanner had a `retrying` state but reconnect never entered it; reconnect now sets session status to `retrying` before the backend handoff | All 5 fixed |
| R22 | Known Issues | Cleared KI-6 at the frontend layer by removing the last misleading restore promise from the archive confirmation dialog and keeping ArchivedThreads in an explicit backend-blocked state. The backend still lacks archived-thread listing and unarchive bindings, but the UI no longer implies that restore works. | Fixed in frontend |
| R23 | Known Issues | Found a frontend regression in model/archive contract clarity: ChatView’s `ModelPicker` still looked like a live thread-model switcher even though it only persists default-model settings, and archive messaging was inconsistent between the settings panel and sidebar confirm dialog. The picker now shows the active thread model separately, labels the editable value as the default for new threads, and toasts when the current thread stays pinned; ArchivedThreads and ThreadRow now use the same one-way archive wording. Playwright browser verification skipped because no `wails dev` process was running. | Fixed in frontend |
