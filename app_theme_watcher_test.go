...
func TestThemeWatcherIgnoresItsOwnAtomicAppearanceWrite(t *testing.T) {
	...
	watcher.suppressMu.Lock()
	watcher.suppressed[filepath.Clean(appearancePath)] = time.Now().Add(-time.Second)
	watcher.suppressMu.Unlock()
...
func TestThemeWatcherSuppressionExpiresAndPrunes(t *testing.T) {
	...
	now := time.Now()
	watcher := &themeWatcher{suppressed: map[string]time.Time{}, now: func() time.Time { return now }}

	stale := filepath.Join(t.TempDir(), "stale.json")
	fresh := filepath.Join(t.TempDir(), "fresh.json")
	watcher.suppress(stale)
	if !watcher.isSuppressed(stale) {
	...
	watcher.suppressMu.Lock()
	remaining := len(watcher.suppressed)
	watcher.suppressMu.Unlock()
	if remaining != 1 {
...
func TestThemeWatcherIgnoresAtomicfileTempNames(t *testing.T) {
	watcher := &themeWatcher{dir: filepath.Clean(t.TempDir())}
	...
	for _, base := range []string{theme.AppearanceFileName, theme.SchemaFileName, theme.TokensFileName, "tokyo-night.json"} {
		...
		if watcher.relevant(filepath.Join(watcher.dir, name)) {
	...
	if !watcher.relevant(filepath.Join(watcher.dir, "tokyo-night.json")) {
...
// boot; the write path must not care.
