...
<div class="flex h-full min-h-0 flex-col">
  ...
  <div class="flex-1 min-h-0 overflow-y-auto space-y-1 pr-1">
    {#if loading}
      <div class="rounded-[var(--radius-control)] border border-border-subtle bg-card/30 px-3 py-2 text-[12px] text-fg-muted">
        Loading…
      </div>
    {:else if discussions.length === 0}
      <div class="rounded-[var(--radius-control)] border border-border-subtle bg-card/30 px-3 py-3 text-[12px] text-fg-muted">
    ...
    {:else}
      {#each discussions as def (keyFor(def))}
        ...
        >
          <span class="mt-0.5 shrink-0 rounded-[var(--radius-field)] border px-1.5 py-0.5 text-[10px] font-semibold uppercase tracking-[0.14em]
          ...
          <span class="min-w-0 flex-1">
            ...
            <span class="mt-0.5 block text-[10px] text-fg-hint tabular-nums">
