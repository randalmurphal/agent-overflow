 ...
 */
export function classifyToolName(toolName: string | undefined | null): ToolKindVisual {
  const raw = (toolName ?? '').trim();
  if (!raw) {
    return { icon: 'generic', label: 'Tool', displayName: 'Tool', isSubagent: false };
  }
  ...
  switch (raw) {
    ...
    case 'Read':
      return { icon: 'eye', label: 'Read', displayName: 'Read', isSubagent: false };
    case 'Grep':
    case 'Glob':
    ...
    case 'Plan':
    case 'ExitPlanMode':
      return { icon: 'checklist', label: 'Plan', displayName: raw, isSubagent: false };
  }

  return { icon: 'generic', label: 'Tool', displayName: raw, isSubagent: false };
}
