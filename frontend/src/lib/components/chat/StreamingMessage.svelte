...
<div class="flex justify-start mb-3">
  <div class="max-w-[85%] rounded-lg px-4 py-2.5 bg-surface-2 text-text-primary">
    ...
    {/if}
    {#if inProgress}
      <p class="whitespace-pre-wrap text-sm leading-relaxed">{inProgress}<span class="inline-block w-1.5 h-4 bg-accent animate-pulse ml-0.5 align-text-bottom"></span></p>
    {:else}
      <span class="inline-block w-1.5 h-4 bg-accent animate-pulse align-text-bottom"></span>
    {/if}
  </div>
</div>
