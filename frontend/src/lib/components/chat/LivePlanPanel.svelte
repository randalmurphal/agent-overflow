<script lang="ts">
  ...
  function rowBarClass(status: PlanStepStatus): string {
...
{#if livePlan}
  ...
  >
    ...
    >
      ...
      <span class="flex flex-col gap-0">
        <span class="inline-flex items-center gap-1.5">
          <span class="font-semibold text-fg-muted">Plan</span>
          {#if inProgressPreview}
            <span class="text-fg-hint/55">·</span>
            <span class="text-fg-default" data-testid="live-plan-in-progress-preview">
              {inProgressPreview}
            </span>
          {/if}
        </span>
    ...
    {#if isExpanded}
      <ul class="mt-0.5 flex flex-col gap-0.5" data-testid="live-plan-list">
