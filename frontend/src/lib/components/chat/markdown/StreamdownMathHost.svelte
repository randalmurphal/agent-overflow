<script lang="ts">
  // Math host. Delegates KaTeX rendering to `lib/markdown/`'s
  ...
  // `lib/markdown/index.ts` does not export `MathToken` — the barrel is
  // exactly what the app imports and nothing more. Inline-shape it here
  // rather than widen that barrel; fields match the renderer's MathToken
  // (type: 'math', raw, text, isInline).
  type MathToken = {
    type: 'math';
    raw: string;
    text: string;
