  ...
  <div class="flex items-center gap-2" role="group" aria-label="Rate limits">
    {#each limits as entry (entry.limitName)}
      <div
        ...
        role="meter"
        aria-valuenow={Math.round(entry.usedPercent)}
        aria-valuemin={0}
        aria-valuemax={100}
        aria-label="{entry.limitName} ({formatWindow(entry.windowMins)})"
      >
        <span class="text-[10px] {textColor(entry.usedPercent)} tabular-nums" aria-hidden="true">{Math.round(entry.usedPercent)}%</span>
        <div class="w-8 h-1.5 rounded-full bg-surface-2 overflow-hidden" aria-hidden="true">
          <div
            class="h-full rounded-full transition-all {barColor(entry.usedPercent)}"
            style="width: {Math.min(entry.usedPercent, 100)}%"
          ></div>
        </div>
        <span class="text-[9px] text-text-secondary/50" aria-hidden="true">{formatWindow(entry.windowMins)}</span>
      </div>
    {/each}
  </div>
{/if}
