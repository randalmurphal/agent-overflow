...
describe('<Composer>', () => {
  ...
  // ---- UP-arrow queue retract (Phase G8) ---------------------------
  //
  // Per Claude TUI's `popAllEditable` behaviour: when the composer is
  // empty and Zone 1 has queued items, plain UP-arrow drops every
  // queued item and merges them into one editable composer draft.

  function seedQueueZone1(threadId: string, messages: { id: string; message: string; attachmentIds?: string[] }[]) {
    replaceQueueForThread(
      threadId,
  ...
  it('UP-arrow retracts queued items into the draft when the textarea is empty', async () => {
    const pane = await buildPane();
    pane.setActiveTurn({ turnId: 't1', turnIndex: 0, startedAt: 0 });
    const draft = await buildDraft();
    seedQueueZone1('thread-1', [{ id: 'q-1', message: 'hello queued' }]);
    const undo = setBindingMock('UndoQueuedItems', async () => [
      {
        id: 'q-1',
        threadId: 'thread-1',
        message: 'hello queued',
        attachmentIds: [],
        sourceProposedPlan: null,
        revisionSourceProposedPlan: null,
        revisionSourceCommentIds: undefined,
    ...
    await waitFor(() => expect(undo).toHaveBeenCalledWith('thread-1'));
    await waitFor(() => expect(draft.content).toBe('hello queued'));
  });

  it('UP-arrow does nothing when the textarea has content', async () => {
    const pane = await buildPane();
    pane.setActiveTurn({ turnId: 't1', turnIndex: 0, startedAt: 0 });
    const draft = await buildDraft();
    draft.setContent('user is typing');
    seedQueueZone1('thread-1', [{ id: 'q-1', message: 'hello queued' }]);
    const undo = setBindingMock('UndoQueuedItems', async () => []);
    ...
    const textarea = getByLabelText('Message Input') as HTMLTextAreaElement;
    textarea.focus();
    // Cursor at start of typed content — still has content, so the
    // predicate should refuse and the keystroke must fall through to
    // native cursor navigation.
    textarea.setSelectionRange(0, 0);
    await fireEvent.keyDown(textarea, { key: 'ArrowUp' });
    await tick();

    expect(undo).not.toHaveBeenCalled();
    expect(draft.content).toBe('user is typing');
  });

  it('UP-arrow does nothing when there are no queue items', async () => {
    const pane = await buildPane();
    const draft = await buildDraft();
    const undo = setBindingMock('UndoQueuedItems', async () => []);
  ...
  it('UP-arrow respects modifier keys (Ctrl+Up does NOT retract)', async () => {
    const pane = await buildPane();
    pane.setActiveTurn({ turnId: 't1', turnIndex: 0, startedAt: 0 });
    const draft = await buildDraft();
    seedQueueZone1('thread-1', [{ id: 'q-1', message: 'hello queued' }]);
    ...
    await fireEvent.keyDown(textarea, { key: 'ArrowUp', metaKey: true });
    await fireEvent.keyDown(textarea, { key: 'ArrowUp', altKey: true });
  ...
  it('UP-arrow with multiple queued items joins them with double-newline', async () => {
    const pane = await buildPane();
    pane.setActiveTurn({ turnId: 't1', turnIndex: 0, startedAt: 0 });
    const draft = await buildDraft();
    seedQueueZone1('thread-1', [
      { id: 'q-1', message: 'first message' },
      { id: 'q-2', message: 'second message' },
      { id: 'q-3', message: 'third message' },
    ]);
    setBindingMock('UndoQueuedItems', async () => [
      {
        id: 'q-1',
        threadId: 'thread-1',
    ...
    await waitFor(() =>
      expect(draft.content).toBe('first message\n\nsecond message\n\nthird message'),
    );
  });

  it('UP-arrow with attachments restores them via ListAttachments lookup', async () => {
    const pane = await buildPane();
    pane.setActiveTurn({ turnId: 't1', turnIndex: 0, startedAt: 0 });
    const draft = await buildDraft();
    seedQueueZone1('thread-1', [
      { id: 'q-1', message: 'with image', attachmentIds: ['att-1'] },
    ]);
    const att = makeAttachment('att-1', 'hero.png');
    ...
    await waitFor(() => expect(draft.content).toBe('with image'));
    expect(draft.attachments.map((a) => a.id)).toEqual(['att-1']);
  });

  it('UP-arrow on RPC failure leaves the draft untouched and surfaces an error', async () => {
    const pane = await buildPane();
    pane.setActiveTurn({ turnId: 't1', turnIndex: 0, startedAt: 0 });
    const draft = await buildDraft();
    seedQueueZone1('thread-1', [{ id: 'q-1', message: 'hello queued' }]);
    ...
    await waitFor(() => expect(pane.generalError).toMatch(/Failed to retract queued messages/));
    expect(draft.content).toBe('');
    // The Zone 1 mirror is not touched on failure — the user can try
    // again. (The backend is the source of truth; if the RPC really
    // landed but the response was lost, a future provider:queue_state_changed
    // event would reconcile.)
    expect(getQueueForThread('thread-1')).toHaveLength(1);
  });

  it('UP-arrow short-circuits gracefully when the queue is dropped before the RPC returns', async () => {
    // Race: predicate sees Zone 1 non-empty, but by the time the
    // backend processes UndoQueuedItems the queue has already drained
    // (e.g. trigger fired). The RPC returns []; the handler must not
    // mutate the draft.
    const pane = await buildPane();
    pane.setActiveTurn({ turnId: 't1', turnIndex: 0, startedAt: 0 });
    const draft = await buildDraft();
    seedQueueZone1('thread-1', [{ id: 'q-1', message: 'racing' }]);
    setBindingMock('UndoQueuedItems', async () => []);
  ...
  it('UP-arrow does nothing when no thread is selected', async () => {
    const pane = createThreadPane();
    const draft = await buildDraft(null);
    const undo = setBindingMock('UndoQueuedItems', async () => []);
    ...
    const { getByLabelText } = render(Composer, { props: { pane, draft } });
    const textarea = getByLabelText('Message Input') as HTMLTextAreaElement;
    // Disabled textarea — focus is a no-op but the keydown handler still
    // runs in tests. The predicate must still refuse on a null threadId.
    await fireEvent.keyDown(textarea, { key: 'ArrowUp' });
    await tick();
