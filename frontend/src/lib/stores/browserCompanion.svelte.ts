...
export function hydrateBrowserCompanionState(threadId: string): void {
  ...
  // The read rides `terminal:operate`, and it runs on every pane thread
  // mount, so a session without that grant would spend one refusal per
  // thread open on a surface it cannot use anyway: the pane is a NATIVE
  // view this machine paints, which no remote client renders. Marking the
  // thread hydrated first is deliberate — the answer for such a session
  // is "no pages", and it will not change until the grant does.
