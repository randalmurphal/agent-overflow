...
describe('hasEngagedItems', () => {
  // The round-INDEPENDENT "logical turn still engaged" fact the content
  // lease's motionImminent needs (utils/scroll/chokepoint.ts):
  // isThreadWorking is per-wire-round and the live-content stamp holds
  // only 500ms, so a quiet subagent wait inside a logical turn is
  // invisible to both. Pinned over the TRANSITIONS, not just the states —
  // the whole point is that it goes false again when the turn settles.

  it('is false with no items', () => {
    expect(createThreadPane().hasEngagedItems()).toBe(false);
  });
  ...
  it('is false when every item has settled', () => {
    ...
    pane.upsertItems([
      ...
      makeItem({ id: 'b', status: 'errored' }),
      makeItem({ id: 'c', status: 'declined' }),
      makeItem({ id: 'd', status: 'killed' }),
    ]);
    expect(pane.hasEngagedItems()).toBe(false);
  });

  it('is true while a tool call is running, and false once it completes', () => {
    // The subagent-outlives-the-round shape: a `running` task row with no
    // open round and nothing advancing.
    const pane = createThreadPane();
    pane.upsertItem(makeItem({ id: 'task', kind: 'tool_call', status: 'running' }));
    expect(pane.hasEngagedItems()).toBe(true);

    pane.upsertItem(makeItem({ id: 'task', kind: 'tool_call', status: 'completed', updatedAt: 1 }));
    expect(pane.hasEngagedItems()).toBe(false);
  });

  it('is true while a block is streaming, and false once it settles', () => {
    const pane = createThreadPane();
    pane.upsertItem(makeItem({ id: 'text', status: 'streaming' }));
    expect(pane.hasEngagedItems()).toBe(true);

    pane.upsertItem(makeItem({ id: 'text', status: 'completed', updatedAt: 1 }));
    expect(pane.hasEngagedItems()).toBe(false);
  });

  it('stays true while ANY item is active, and only falls on the last one', () => {
    const pane = createThreadPane();
    pane.upsertItems([
      makeItem({ id: 'done', status: 'completed' }),
      makeItem({ id: 'task', itemIndex: 1, kind: 'tool_call', status: 'running' }),
      makeItem({ id: 'text', itemIndex: 2, status: 'streaming' }),
    ]);
    expect(pane.hasEngagedItems()).toBe(true);

    pane.upsertItem(makeItem({ id: 'text', itemIndex: 2, status: 'completed', updatedAt: 1 }));
    expect(pane.hasEngagedItems()).toBe(true); // the running task still holds it

    pane.upsertItem(
      makeItem({ id: 'task', itemIndex: 1, kind: 'tool_call', status: 'errored', updatedAt: 1 }),
    );
    expect(pane.hasEngagedItems()).toBe(false);
  });

  it('is false again after the pane is cleared', () => {
    // A pane emptied mid-stream (clear-pane, "+ New") must not leave the
    // lease held by items that are no longer loaded.
    const pane = createThreadPane();
    pane.upsertItem(makeItem({ id: 'text', status: 'streaming' }));
    expect(pane.hasEngagedItems()).toBe(true);
    pane.clear();
