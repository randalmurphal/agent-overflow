...
describe('applyUserMessageReverted', () => {
  ...
  it('fences queued and delayed pre-cut frames while preserving the replacement', async () => {
    ...
    applyItemStreamEvent({ action: 'upsert', threadId: 'thread-a', item: old }, 10);
    ...
    applyItemStreamEvent({ action: 'upsert', threadId: 'thread-a', item: old }, 11);
    applyItemStreamEvent({ action: 'upsert', threadId: 'thread-a', item: replacement }, 12);
    flushItemEventQueue();
    applyUserMessageReverted(cut);
  ...
  it('fences an unsequenced frame the queue already held when the cut arrived', async () => {
    const pane = await seedPane();
    const old = makeItem({ id: 'old-output', threadId: 'thread-a', turnIndex: 1, itemIndex: 1, kind: 'assistant_text' });
    applyItemStreamEvent({ action: 'upsert', threadId: 'thread-a', item: old });
    applyUserMessageReverted({ threadId: 'thread-a', userItemId: 'u:1', turnIndex: 1, historyEpoch: 2, historyRev: 20, itemEventSequence: 11 });
    const later = makeItem({ id: 'new-output', threadId: 'thread-a', turnIndex: 1, itemIndex: 2, kind: 'assistant_text' });
    applyItemStreamEvent({ action: 'upsert', threadId: 'thread-a', item: later }, 12);
    flushItemEventQueue();
    expect(pane.getItemById('old-output')).toBeUndefined();
    expect(pane.getItemById('new-output')).toBeDefined();
  });
...
it('fences turn events that arrive after the RPC cut and allows the replacement turn', async () => {
  ...
  applyTurnStarted({ threadId: 'thread-a', turnId: 'old', turnIndex: 1, startedAt: 1 }, 8);
  expect(isThreadWorking('thread-a')).toBe(false);
  applyTurnStarted({ threadId: 'thread-a', turnId: 'new', turnIndex: 1, startedAt: 2 }, 9);
  expect(isThreadWorking('thread-a')).toBe(true);
  applyTurnCompleted({ threadId: 'thread-a', turnId: 'old', turnIndex: 1, startedAt: 1, completedAt: 3, aborted: true } as any, 7);
  expect(isThreadWorking('thread-a')).toBe(true);
