...
- `methodRoutes.ts` is GENERATED by `methodgen` from the same scan that
  ...
  it; a Go test byte-checks the two. Five routes: `thread` and `project`
  resolve argument 0 through the entity index, `home` is the page's own
  backend, `selected` is the machine the person is looking at
  (`stores/selectedBackend.svelte.ts`), and `all` fans out and merges.
