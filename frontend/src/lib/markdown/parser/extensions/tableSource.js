...
const normalizeCarriageReturns = (source) => source.replace(/\r\n|\r/g, '\n');

const originalRawForNormalizedLength = (source, normalizedLength) => {
    let sourceIndex = 0;
...
export const parseTableSource = (src, detectFooter) => {
...
export const parseTableBlockSource = (src, detectFooter) => {
    const match = matchTableSource(src);
    if (!match)
        return null;
