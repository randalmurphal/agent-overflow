 ...
 */
const consumedPrefix = (src, match, raw) => src.slice(0, Math.min(raw.length, trimmedNewlineEnd(match)));
export const markedAlert = {
    ...
    tokenizer(src) {
        ...
        processAlertToken(blockquoteToken, this.lexer.options.tokenizer);
