...
import { markedList, markedListBlock, parseListSource, tokenizeListItemContent } from './extensions/list';
import { markedBr } from './extensions/br';
import { markedHr } from './extensions/hr';
import { markedTable, tokenizeTableTail } from './extensions/table';
import { markedTableBlock, parseTableBlockSource } from './extensions/tableSource';
...
import { markedBlockquoteBlock, parseBlockquoteSource } from './extensions/blockquoteSource';
...
const blockExtensionsOf = (extensions: Extension[]): Extension[] | null => {
...
const sameExtensionSequence = (left: Extension[] | null, right: Extension[] | null): boolean => {
    if (left === right)
        return true;
...
const getLexOptions = (extensions: Extension[]): MarkedOptions => {
    if (extensions.length === 0)
...
const getBlockOptions = (extensions: Extension[]): MarkedOptions => {
...
const truncateParseBlocksCache = (cache: ParseBlocksCache, rawLength: number): void => {
    let blockLength = 0;
...
const appendParseBlockRaw = (cache: ParseBlocksCache, raw: string, keep: boolean, start: number): number => {
...
const replaceParseBlockRaw = (cache: ParseBlocksCache, rawIndex: number, raw: string): void => {
...
const trailingBlockMayMergeBackward = (cache: ParseBlocksCache): boolean => {
...
const paragraphAppendSafe = (raw: string): boolean => {
    if (/[\r\n]/.test(raw))
        return false;
...
const isKeptType = (type: string): boolean => type !== 'space' && type !== 'footnote';
...
const openFenceInfo = (source: string): OpenFence | null => {
...
const appendDeltaOf = (markdown: string, cache: ParseBlocksCache, provenAppend: ProvenAppend | undefined): string | null => {
...
const sealedLengthOf = (list: BlockToken): number => {
...
const lastLineStartOf = (source: string): number => {
    let end = source.length;
    while (end > 0 && source[end - 1] === '\n')
        end--;
    return source.lastIndexOf('\n', end - 1) + 1;
};
const tablePrefixLength = (table: BlockToken, source: string): number | null => {
    if (table.type !== 'table')
        return null;
...
const tableAppendInfo = (table: BlockToken, source: string): TableAppendInfo | null => {
...
const ROWSPAN_CARET = /\^[ \t]*(\||\n|$)/;
const tableTailUnsafe = (slice: string): boolean => ROWSPAN_CARET.test(slice) || FOOTER_MARKER_LINE.test(slice);
...
const hasTrailingRowspanCaret = (source: string): boolean => {
...
const commitLexSource = (cache: IncrementalLexCache, block: string, trim: TrimmedBlock): void => {
...
const updateTrailingBlockRecord = (
...
const reuseUnchangedTokens = <T extends ReusableToken>(previous: T[], next: T[]): T[] => {
...
type LinkTable = TokensList['links'];
