...
describe('parkedStopFromItem', () => {
  it('reads a parked sibling and nothing else', () => {
    ...
    expect(parkedStopFromItem({ status: 'parked', meta })).toEqual({ waitingOn: 2, reportItemId: 'r1', runStartedAt: 500, woke: true });
  ...
  it('reads a sibling with no run facts as an unwoken run with no report', () => {
    expect(parkedStopFromItem({ status: 'parked', meta: '' })).toEqual({ waitingOn: 0, reportItemId: '', runStartedAt: 0, woke: false });
  });
});

describe('parkedStopSentence', () => {
  const stop = { reportItemId: '', runStartedAt: 0 };
