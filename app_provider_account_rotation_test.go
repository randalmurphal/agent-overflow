...
func TestClaudeProbeConfigAlwaysWiresTheRotationReader(t *testing.T) {
	...
	app := &App{providerCredentials: credentials}
	...
	unpinned := app.claudeProbeConfig("claude", nil)
	...
	cfg := app.claudeProbeConfig("claude", map[string]string{"CLAUDE_CONFIG_DIR": pinnedHome})
...
func TestClaudeProbeConfigIsTheOnlyProbeConstructor(t *testing.T) {
	const allowed = "app_provider_account_env.go"
	...
	walkErr := filepath.WalkDir(".", func(path string, entry os.DirEntry, err error) error {
		...
		if strings.Contains(string(source), needle) {
			t.Errorf(
				"%s builds a %s literal; use (*App).claudeProbeConfig so the probe carries "+
...
func TestProviderCredentialChainPositionReadsOnlyClaudeOAuth(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			got, ok := providerCredentialChainPosition(tc.provider, []byte(tc.data))
