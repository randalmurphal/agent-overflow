...
func TestCodexThreadCostReadRefusesAReplacedSession(t *testing.T) {
	...
	app.mu.Lock()
	// A live session under a DIFFERENT token than the settled turn's, and
	// with no codex handle — either alone must stop the read before any wire
	// work is attempted.
	app.sessions[id] = session{provider: string(provider.Codex), token: "token-new"}
	app.mu.Unlock()
