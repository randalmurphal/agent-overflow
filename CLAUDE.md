...
| Target | What it does |
|---|---|
| `make install` | `wails3` CLI (via the `go.mod` tool directive) + pnpm deps + Playwright Chromium. First-time setup only: every build and test target reinstalls the frontend deps itself when `frontend/pnpm-lock.yaml` is newer than what pnpm last installed. |
...
| `make e2e-android` | the shell smoke, driving the app inside a running emulator's own WebView (Playwright's Android API), where the native seams are real. Its own config, directory and one spec; first run on the Mac 2026-09-03 against an arm64 android-36 emulator. Not a blocking gate: with none attached it prints how to start one and exits clean. See [e2e/AGENTS.md](e2e/AGENTS.md) § The emulator smoke. |
