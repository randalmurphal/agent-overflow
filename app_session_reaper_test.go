...
func TestRecordSessionActivityBumpsOnEveryEventKind(t *testing.T) {
	...
	prev := app.sessions[thread.ID].liveness.lastActivityUnixNano.Load()
	for i, k := range kinds {
		...
		got := app.sessions[thread.ID].liveness.lastActivityUnixNano.Load()
		if got <= prev {
			t.Fatalf("kind[%d]=%s did not advance lastActivity (prev=%d got=%d)", i, k, prev, got)
		}
		prev = got
	}
	if got := app.sessions[thread.ID].liveness.activeTurns.Load(); got != 0 {
...
func TestRecordSessionActivityTracksActiveTurns(t *testing.T) {
	...
	l := app.sessions[thread.ID].liveness
...
func TestRecordSessionActivityDisconnectDrainsActiveTurns(t *testing.T) {
	...
	l.activeTurns.Store(1) // simulate stuck counter
	app.sessions[thread.ID] = session{
	...
	for i := 0; i < 4; i++ {
		...
		go func(slot int) {
			...
			for {
				...
				app.mu.Lock()
				app.sessions[churnID] = session{
					provider: string(provider.Codex),
					token:    "churn",
					liveness: newSessionLiveness(time.Now()),
				}
				delete(app.sessions, churnID)
				app.mu.Unlock()
			}
		}(i)
	}
