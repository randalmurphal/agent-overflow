...
func ApplyDefaults(p RunPlan) RunPlan {
	// macOS presents its root-owned temp aliases as /var and /tmp while
	// filesystem identity resolves them under /private. Normalize only those
	// OS aliases before deriving artifact paths; caller-created symlinks below
	// them remain visible to the later safety checks.
	p.DataRoot = normalizePlanPath(p.DataRoot)
	p.Output = normalizePlanPath(p.Output)
	p.Capsule = normalizePlanPath(p.Capsule)
	...
	for i := range p.Artifacts {
		p.Artifacts[i].Destination = normalizePlanPath(p.Artifacts[i].Destination)
	}
	if p.Adapter == "" {
