...
macOS's root-owned `/var` and `/tmp` aliases normalize to `/private/...` before
plan-derived paths are frozen. This does not resolve caller-created symlinks
below those aliases; the ordinary component checks must still reject them.
