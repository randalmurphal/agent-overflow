...
| `FormatAttachments(attachments) string` | Renders one bullet per attachment with filename, MIME type, and byte size, for the prompt's metadata section. BOTH kinds: a title is about what the turn was, and "the user attached report.pdf" is as much of that as an image is. Only the image PATH list (`threadtitleapp`) filters, because only an image can be shown to the model. |
