...
| `Meta.SendID` (`sendId`) | The client's idempotency id for the send that WROTE this row, and the reason a persisted user_text row can answer a question about a live RPC. A frame re-sent over a replacement socket carries the id its first attempt did, so the backend recognises the repeat by scanning the thread's newest reader-authored rows (`store.RecentUserTextItems`) rather than keeping an id table of its own. Empty is legal and means the send predates the id or came from a path that mints none, in which case nothing dedupes. |
