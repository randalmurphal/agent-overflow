...
| Symbol | Purpose |
|---|---|
| `Event` | Wire shape for `provider:status`. JSON tags pin the field names the frontend reads (`provider`, `status`, `message`, `version`, `actionable`, `actionUrl`). The matching TS interface in `frontend/src/lib/types/events.ts` is hand-written, so keep both in sync. |
