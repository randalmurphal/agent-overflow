...
var composerRepair = []composerRow{
	...
	{"provider-retries-exhausted → run resume", "continues the parked attempt on the session its turn died in once the transient provider failure clears"},
	{"provider-usage-limited → run resume", "tries the parked attempt immediately; use after usage resets or after switching accounts, and no recorded limit blocks the attempt"},
	{"loop-limit-exhausted → run resume --phase <id>", "naming an earlier phase re-enters the cycle from outside and refills its bound"},
	{"retries-exhausted (legacy) → inspect cause", "run resume preserves the old continuation behavior; use --phase <id> for a spent loop bound"},
