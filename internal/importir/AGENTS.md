...
Two types. `Event` is a `provider.ProviderEvent` plus the `SourceUUID` /
`SourceOffset` coordinates of the session-file line it was read from.
`Warning` is a `{Code, Message}` finding surfaced next to its session.
...
- Do NOT let this package import `store`, `triage`, or a provider
  ...
  path back to SQLite. Provider packages never import store or triage
  (see `internal/AGENTS.md`).
