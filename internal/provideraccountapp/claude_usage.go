...
func (m *Manager) rateLimitProbeClient() *http.Client {
	if m.deps.HTTPClient != nil {
		if client := m.deps.HTTPClient(); client != nil {
			return client
		}
	}
...
var (
	...
	// ErrClaudeCredentialsExpired is exposed for root integration assertions.
	ErrClaudeCredentialsExpired = errClaudeCredentialsExpired
)
...
func (m *Manager) probeSelectedClaudeRateLimits(
	ctx context.Context,
	selection Selection,
	credential []byte,
	...
	refreshCfg := m.claudeProbeConfig(m.providerBinaryPath(string(provider.Claude)), nil)
	...
	refreshed, readErr := m.credentials.ReadCredentialSnapshot(
	...
	if err := m.assertSelectedClaudeIdentity(selection, info); err != nil {
...
func (m *Manager) assertSelectedClaudeIdentity(
	selection Selection,
...
// AssertSelectedClaudeIdentity exposes the rotation attribution guard for
// focused root integration tests.
func (m *Manager) AssertSelectedClaudeIdentity(selection Selection, info provider.AccountInfo) error {
	return m.assertSelectedClaudeIdentity(selection, info)
}
...
func (m *Manager) probeInactiveClaudeRateLimits(
	ctx context.Context,
	credential []byte,
