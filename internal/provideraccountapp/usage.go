...
func (m *Manager) RefreshProviderAccountUsage(providerName, accountID string) error {
	return m.refreshProviderAccountUsage(m.context(), providerName, accountID)
}

// RefreshProviderAccountUsageContext is the periodic-poll adapter; Manager
// still owns the credential transaction while root owns cadence/cancellation.
func (m *Manager) RefreshProviderAccountUsageContext(ctx context.Context, providerName, accountID string) error {
	return m.refreshProviderAccountUsage(ctx, providerName, accountID)
}

func (m *Manager) refreshProviderAccountUsage(
	ctx context.Context,
	providerName,
	accountID string,
) (retErr error) {
	...
	backoffHold := func() error {
		if m.deps.Backoff != nil {
			if remaining := m.deps.Backoff.Remaining(providerName, accountID); remaining > 0 {
				return fmt.Errorf(
	...
	m.mu.RLock()
	account, exists := m.store.Get(providerName, accountID, time.Now())
	if !exists {
		m.mu.RUnlock()
		return fmt.Errorf("%s account %q is not saved", providerName, accountID)
	}
	active, isActive := m.store.Active(providerName, time.Now())
	...
	var err error
	if isSelected {
		selection = m.selectionLocked(providerName)
		credential, err = m.credentials.ReadCredentialSnapshot(
			providerName,
			"",
			true,
		)
	...
	defer func() {
		if probeRan {
			if m.deps.Backoff != nil {
				m.deps.Backoff.Note(providerName, accountID, probeErr)
			}
		}
	}()
	...
	if providerName == string(provider.Codex) {
		ephemeral, err = m.credentials.NewEphemeralHomeWithCredential(
			providerName,
			credential.Data,
			accountID,
		)
	...
	case string(provider.Claude):
		...
		if refresh.refreshesInCanonicalHome() {
			snapshot, refresh.refreshed, probeErr = m.probeSelectedClaudeRateLimits(
				ctx,
				selection,
				credential.Data,
			)
			...
			snapshot, probeErr = m.probeInactiveClaudeRateLimits(ctx, credential.Data)
			if probeErr != nil {
				return mapProviderAccountLoginError(providerName, account, probeErr)
			}
		}
	case string(provider.Codex):
		probeCfg := m.codexProbeConfig(
			m.providerBinaryPath(providerName),
		...
		refreshed, readErr := m.credentials.ReadEphemeralCredential(ephemeral)
		if readErr != nil {
	...
	if !refresh.isSelected &&
		...
		refresh.slotPersisted = m.credentials.WriteAccountCredential(
	...
	m.mu.Lock()
	commit, commitErr := m.commitProviderUsageRefreshLocked(refresh)
	m.mu.Unlock()
...
// caller holds mu for the whole call.
func (m *Manager) commitProviderUsageRefreshLocked(
	refresh providerUsageRefresh,
) (providerUsageCommit, error) {
	commit := providerUsageCommit{generation: refresh.generation}
	if err := m.validateProviderUsageRefreshSelectionLocked(
		refresh.providerName,
		refresh.accountID,
		refresh.generation,
		refresh.isSelected,
	...
	if refresh.providerName == string(provider.Codex) {
		observedAccountID, updated, err := m.accountIDForObservedIdentity(
			refresh.providerName,
			refresh.accountID,
			refresh.observed,
		)
	...
	if bytes.Equal(latest, canonical) {
		...
		if err := m.credentials.CommitSelectedCredential(
			refresh.providerName,
			refresh.accountID,
			latest,
		); err != nil {
			delete(m.fingerprints, refresh.providerName)
			return commit, err
		}
		if err := m.verifyCanonicalProviderCredentialLocked(
			refresh.providerName,
	...
	if refresh.providerName == string(provider.Codex) && !bytes.Equal(latest, refresh.probed) {
		advanced, err := m.store.AdvanceActiveCredential(
			refresh.providerName,
			refresh.accountID,
		)
		if err != nil {
		...
		commit.generation = m.store.Generation(refresh.providerName)
		commit.account = &advanced
	}
	m.rememberProviderCredentialFingerprintLocked(refresh.providerName, latest)
	return commit, nil
}
...
func (m *Manager) validateProviderUsageRefreshSelectionLocked(
	...
	if m.store.Generation(providerName) != generation {
		return fmt.Errorf("%s account changed while refreshing usage; retry", providerName)
	}
	if _, exists := m.store.Get(providerName, accountID, time.Now()); !exists {
