...
// worth delivering at all, live in wake_delivery.go.
...
func (s *Service) afterWorkflowResting(item store.WorkItem, usageClaim *store.WorkflowProviderUsageAttentionClaim) bool {
	if item.OriginThreadID == "" {
		return false
	}
	composed, err := s.composeWorkflowWake(item, nil)
	if err != nil {
		s.deps.Logf("workflow wake %s: compose: %v", item.ID, err)
		s.releaseWorkflowUsageAttention(usageClaim)
		return false
	}
	composed.usageAttention = usageClaim
	return s.deliverWorkflowWake(item, composed)
}
...
// duplicate.
func (s *Service) surfaceDescendantPark(child store.WorkItem) UsageAttentionSurface {
	chain, err := s.Ancestry(child)
	if err != nil {
		s.deps.Logf("workflow wake %s: resolve root: %v", child.ID, err)
	...
	usage := s.UsageAttentionForRest(root, child)
	return s.surfaceResolvedDescendantPark(child, chain, usage)
}

func (s *Service) surfaceResolvedDescendantPark(
	child store.WorkItem,
	chain []store.WorkItem,
	usage UsageAttentionDecision,
) UsageAttentionSurface {
	root := chain[0]
	...
	if engine.Reason(child.Reason) == engine.ReasonGate {
		descendant.run.GateDecision, descendant.run.GateLabel = s.GateDecision(child.ID)
	}
	phase, envelope, err := s.workflowRestingPhase(child.ID)
	if err != nil {
		s.deps.Logf("workflow wake %s: read parked phase: %v", child.ID, err)
	} else {
		...
		descendant.outputs, descendant.overflow = s.wakeAttemptOutputs(child, phase, envelope)
	}
	...
	go s.sendWorkflowItemNotification(root, digest)
	if root.OriginThreadID == "" {
		return usage.Surface
	}
	composed, err := s.composeWorkflowWake(root, descendant)
	if err != nil {
		s.deps.Logf("workflow wake %s: compose descendant park: %v", root.ID, err)
		s.releaseWorkflowUsageAttention(usage.Claim)
		return UsageAttentionSurface{}
	}
	composed.usageAttention = usage.Claim
	if !s.deliverWorkflowWake(root, composed) && usage.Claim != nil {
		return UsageAttentionSurface{}
	}
	return usage.Surface
}
...
func (s *Service) Ancestry(item store.WorkItem) ([]store.WorkItem, error) {
	...
	for depth := 0; current.ParentItemID != ""; depth++ {
		...
		parent, err := s.deps.Store.GetWorkItemSummary(current.ParentItemID)
		if err != nil {
...
func (s *Service) composeWorkflowWake(item store.WorkItem, descendant *wakeDescendant) (composedWake, error) {
	input := wake.Input{
		Run: wake.Run{
	...
	if descendant != nil {
		...
		input.Run.GateDecision, input.Run.GateLabel = s.GateDecision(item.ID)
	}
	timeline, err := s.deps.Store.ListWorkItemPhaseTimeline(item.ID)
	if err != nil {
	...
	if descendant == nil {
		if phase, ok := currentWorkflowPhaseTimelineAttempt(timeline); ok {
			...
			input.AttemptOutputs, input.AttemptOutputOverflow = s.wakeAttemptOutputs(item, phase, phase.OutputEnvelope)
		}
	...
	references, err := s.wakeReferences(item, timeline, input.Descendant)
	if err != nil {
...
func (s *Service) wakeAttemptOutputs(
	...
	if err != nil {
		s.deps.Logf("workflow wake %s: digest parked attempt outputs: %v", item.ID, err)
		return nil, 0
	}
...
func (s *Service) wakeReferences(
	...
	if descendant != nil {
		...
		if narrative, ok := s.workflowRestingNarrative(descendant.ItemID); ok {
		...
		failed, err := s.workflowFailedUnitReferences(descendant.ItemID, "called run failed unit")
		if err != nil {
			return nil, err
		}
	...
	if phase, ok := currentWorkflowPhaseTimelineAttempt(timeline); ok {
		if narrative, present := workflowhost.NarrativeReference(
			s.dataRoot(), item.ID, phase.PhaseID, phase.Attempt,
		); present {
	...
	failed, err := s.workflowFailedUnitReferences(item.ID, "failed unit")
	if err != nil {
		return nil, err
	}
...
const MaxFailedUnitNoteRunes = maxFailedUnitNoteRunes
...
func (s *Service) workflowFailedUnitReferences(itemID, label string) ([]wake.Reference, error) {
	units, err := s.FailedUnits(itemID)
	if err != nil {
		return nil, err
	}
	var references []wake.Reference
...
func (s *Service) workflowRestingNarrative(itemID string) (string, bool) {
	phase, _, err := s.workflowRestingPhase(itemID)
	if err != nil || phase.PhaseID == "" {
		return "", false
	}
	return workflowhost.NarrativeReference(s.dataRoot(), itemID, phase.PhaseID, phase.Attempt)
}
...
func (s *Service) GateDecision(itemID string) (kind, label string) {
	rows, err := s.deps.Store.ListWorkItemPhaseProvenance(itemID)
	if err != nil {
		s.deps.Logf("workflow wake %s: read gate decision: %v", itemID, err)
		return "", ""
	}
	for index := len(rows) - 1; index >= 0; index-- {
		...
		if err := json.Unmarshal(rows[index].GateTrace, &trace); err != nil {
			s.deps.Logf("workflow wake %s: decode gate trace %s/%d: %v",
...
func (s *Service) workflowRestingPhase(itemID string) (store.WorkItemPhaseTimeline, json.RawMessage, error) {
	timeline, err := s.deps.Store.ListWorkItemPhaseTimeline(itemID)
	if err != nil {
