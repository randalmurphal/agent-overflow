...
func (s *Service) afterWorkflowGateNotify(event engine.NotifyEvent) {
	s.wake.Go(func() { s.surfaceWorkflowGateNotify(event) })
}

func (s *Service) surfaceWorkflowGateNotify(event engine.NotifyEvent) {
	item, err := s.deps.Store.GetWorkItem(event.ItemID)
	if err != nil {
		s.deps.Logf("workflow progress wake %s: load run: %v", event.ItemID, err)
		return
	}
	root := item
	...
	if item.ParentItemID != "" {
		...
		if err != nil {
			s.deps.Logf("workflow progress wake %s: resolve root: %v", item.ID, err)
			return
		}
		root = chain[0]
	...
	input.Outputs, input.OutputOverflow = s.workflowGateOutputs(item, event)
	message := wake.ComposeProgress(input)
	if len(message) > maxWakeMessageBytes {
		s.deps.Logf("workflow progress wake %s: composed %d bytes; maximum is %d",
			root.ID, len(message), maxWakeMessageBytes)
		return
	}
	s.deliverWorkflowWake(root, composedWake{signature: wake.ProgressSignature(input), message: message})
}
...
// elaboration.
func (s *Service) workflowGateOutputs(item store.WorkItem, event engine.NotifyEvent) ([]wake.Output, int) {
	timeline, err := s.deps.Store.ListWorkItemPhaseTimeline(item.ID)
	if err != nil {
		s.deps.Logf("workflow progress wake %s: list phase timeline: %v", item.ID, err)
		return nil, 0
	}
	for _, phase := range timeline {
		...
		if err != nil {
			s.deps.Logf("workflow progress wake %s: digest gate outputs: %v", item.ID, err)
			return nil, 0
		}
