...
func TestHarnessListMethodsReportsTheWholeWireSurface(t *testing.T) {
	...
	appMethods, err := dispatcher.Register(&methodTestApp{}, transport.RegisterOptions{
		Package:   "main",
		TypeName:  "App",
	...
	for _, forbidden := range []string{"PageMarker", "Seed", "SetInstanceRemoval", "SetSoakAutopilot", "SetWireMethods"} {
		if slices.Contains(got, forbidden) {
			t.Errorf("internal boot helper %q leaked onto the Harness RPC surface", forbidden)
		}
	}
...
type methodTestApp struct{}

func (*methodTestApp) ListThreads() []string { return []string{} }
