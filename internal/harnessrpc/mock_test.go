...
func TestHarnessScenarioRulesResolveAndFallBack(t *testing.T) {
	...
	if claude.FixtureRoot != h.config.DataRoot {
...
func TestHarnessSetScenarioValidation(t *testing.T) {
	...
	fixturePath := filepath.Join(h.config.DataRoot, "ok.ndjson")
	...
	if _, err := h.HarnessSetScenario(HarnessScenarioSpec{Scenario: fixtureScenario(h.config.DataRoot)}); err == nil {
...
func TestHarnessControlRoundTrip(t *testing.T) {
	...
	host.emit = func(channel eventchan.Channel, data any) {
		if channel != eventchan.HarnessMock {
			return
		}
		mu.Lock()
		defer mu.Unlock()
	...
	controlServer, extra, err := StartControl(h)
	if err != nil {
	...
	if extra[control.EnvAddr] == "" || extra[control.EnvToken] == "" {
		t.Fatalf("provider env missing control credentials: %+v", extra)
	}
...
func TestHarnessClearThreadProviderCursorRequiresAnIdleResumableThread(t *testing.T) {
	h, host := newHarnessTestHost(t)
	seedHarnessThread(t, host.store, "thread-cursor")
	if _, err := host.store.UpdateSessionRef("thread-cursor", "provider-session"); err != nil {
		t.Fatal(err)
	}
	...
	if err := host.store.UpdateTurnCompleted("thread-cursor:1", time.Now().UnixMilli(), "end_turn", "", "", ""); err != nil {
		t.Fatal(err)
	}
	if err := h.HarnessClearThreadProviderCursor("thread-cursor"); err != nil {
		t.Fatalf("clear idle cursor: %v", err)
	}
	thread, err := host.store.GetThread("thread-cursor")
	if err != nil {
		t.Fatal(err)
	}
...
func TestStartControlAlwaysPinsTheMockTranscriptHome(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			got := env[control.EnvTranscriptHome]
