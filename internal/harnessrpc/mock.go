// mock.go — the mock-provider control surface: scenario
...
// ControlServer owns the loopback listener lifetime separately from the RPC
// receiver so lifecycle methods do not accidentally join the wire surface.
type ControlServer struct{ server *control.Server }

func StartControl(h *Harness) (*ControlServer, map[string]string, error) {
	if h == nil {
		return nil, nil, fmt.Errorf("harness receiver unavailable")
	}
	...
	h.control = srv
	h.mu.Unlock()
	return &ControlServer{server: srv}, env, nil
}
...
func (h *Harness) onMockReport(info control.MockInfo, rep control.Report) {
	...
	h.config.Host.Emit(eventchan.HarnessMock, harnessMockEvent{
		MockID:   info.MockID,
...
func (h *Harness) HarnessClearThreadProviderCursor(threadID string) error {
	...
	if turn, active, err := database.GetActiveTurn(threadID); err != nil {
	...
	if _, err := database.UpdateSessionRef(threadID, ""); err != nil {
		return fmt.Errorf("clear provider cursor for thread %s: %w", threadID, err)
	}
	return nil
}
