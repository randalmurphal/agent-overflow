...
// newUIQueryHarness builds the smallest Harness that can emit through its
// production Host seam. Nothing here touches the store or a provider.
func newUIQueryHarness(t *testing.T) (*Harness, *testHost, <-chan harnessUIQueryEvent) {
	t.Helper()
	emitted := make(chan harnessUIQueryEvent, 8)
	host := &testHost{}
	host.emit = func(channel eventchan.Channel, data any) {
		if channel != eventchan.HarnessUIQuery {
			return
		}
	...
	return New(Config{Host: host}), host, emitted
}
...
func TestHarnessUIQueryResolvesOnItsReply(t *testing.T) {
	h, _, emitted := newUIQueryHarness(t)
