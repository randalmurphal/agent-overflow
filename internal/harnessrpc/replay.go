// replay.go — wire-level replay RPCs: re-emit recorded
...
func (h *Harness) replayerEngine() *harness.Replayer {
	...
	if h.replayer == nil {
		h.replayer = harness.NewReplayer(
			...
			func(kind string, data json.RawMessage) {
				if h.config.Host != nil {
					h.config.Host.Emit(eventchan.Channel(kind), data)
				}
			},
			func(st harness.ReplayStatus) {
				if h.config.Host != nil {
					h.config.Host.Emit(eventchan.HarnessReplay, st)
				}
			},
		)
	}
...
func (h *Harness) HarnessRecordStart(name, threadID string) (harnessRecording, error) {
	...
	replayLog := h.replayLog()
	if replayLog == nil || !replayLog.Enabled() {
		return harnessRecording{}, fmt.Errorf("event log is disabled; harness boots enable it — was the setting changed?")
	}
	database := h.store()
	if database == nil {
		return harnessRecording{}, fmt.Errorf("store unavailable")
	}
	if turn, ok, err := database.GetActiveTurn(threadID); err != nil {
	...
	dir := filepath.Join(h.config.DataDir, "bundles", name)
	...
	defer cancel()
	if err := replayLog.WaitForDrain(ctx); err != nil {
	...
	if err := database.SnapshotTo(filepath.Join(dir, "db.snapshot")); err != nil {
		return fail(err)
	}
	...
	rec := harnessRecording{
		Name:          name,
		ThreadID:      threadID,
		StartedAt:     h.config.Now().UnixMilli(),
		offset:        offset,
		dir:           dir,
		logAtStart:    logAtStart,
		backupAtStart: backupAtStart,
		lostAtStart:   replayLog.LostCount(),
	}
	h.mu.Lock()
	if h.recording != placeholder {
...
func (h *Harness) HarnessRecordStop() (HarnessBundleMeta, error) {
	...
	if lost := replayLog.LostCount() - rec.lostAtStart; lost > 0 {
	...
	meta := HarnessBundleMeta{
		...
		CreatedAt:  h.config.Now().UnixMilli(),
		EventCount: events,
		SnapshotAt: "start",
	}
...
func (h *Harness) HarnessReplayBundle(name string, opts harness.ReplayOptions) (harness.ReplayStatus, error) {
	...
	database := h.store()
	if database == nil || h.config.Host == nil {
		return harness.ReplayStatus{}, fmt.Errorf("harness host or store unavailable")
	}
	...
	identity, err := database.RestoreFrom(filepath.Join(dir, "db.snapshot"))
	if err != nil {
		return harness.ReplayStatus{}, err
	}
	...
	h.config.Host.PublishStoreIdentity(identity)
...
func (h *Harness) HarnessListBundles() ([]HarnessBundleMeta, error) {
	root := filepath.Join(h.config.DataDir, "bundles")
...
func (h *Harness) eventLogPath(threadID string) string {
	return filepath.Join(h.config.DataDir, "replay", threadID+".jsonl")
}
