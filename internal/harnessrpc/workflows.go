...
func (h *Harness) seedProjectWorkflowFiles(projectRow store.Project, spec HarnessSeedWorkflows) error {
	configDir := projectconfig.ConfigDir(h.config.DataDir, projectRow.Slug)
	...
	for _, workflowID := range workflowIDs {
		if err := h.config.Host.ResolveProjectWorkflow(projectRow.ID, workflowID); err != nil {
...
func (h *Harness) startSeedWorkflowItem(projectID string, spec HarnessSeedWorkflowItem, seeds json.RawMessage, target engine.State) (string, error) {
	if target == engine.StateRunning {
		item, err := h.config.Host.StartWorkflowRun(projectID, spec.Workflow, spec.Goal, seeds, spec.StepMode)
		if err != nil {
			return "", err
		}
		return item.ID, nil
	}
	subscriber, err := h.config.Host.SubscribeEvents()
	if err != nil {
...
func (h *Harness) awaitSeedWorkflowTarget(subscriber *transport.Subscriber, itemID string, target engine.State) error {
	...
	for {
		...
		case <-timer.C:
			item, getErr := h.store().GetWorkItem(itemID)
			if getErr != nil {
...
func (h *Harness) prepareWorkflowReset() (func() error, error) {
	if h.config.Host == nil || !h.config.Host.HasWorkflowEngine() {
		return nil, nil
	}
	if err := h.config.Host.SetWorkflowGlobalPause(true); err != nil {
		return nil, fmt.Errorf("reset workflows: pause engine: %w", err)
	}
	resume := func() error {
		if err := h.config.Host.SetWorkflowGlobalPause(false); err != nil {
	...
	database := h.store()
	if database == nil {
		return nil, fmt.Errorf("reset workflows: store unavailable")
	}
	running, err := database.ListWorkItemSummaries(store.WorkItemListFilter{
	...
	for _, item := range running {
		if err := h.config.Host.CancelWorkflowItem(context.Background(), item.ID); err != nil {
	...
	if err := h.config.Host.SyncWorkflowEngine(); err != nil {
