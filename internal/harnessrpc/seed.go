// seed.go — HarnessSeed / HarnessReset: declarative
...
// workflows.go and never write workflow persistence directly.
package harnessrpc
...
// Seed is the typed in-process companion used by App integration tests. It is
// a free function so it cannot accidentally join the reflected RPC surface;
// wire callers use HarnessSeed and therefore retain strict JSON decoding.
func Seed(h *Harness, spec HarnessSeedSpec) (HarnessSeedResult, error) {
	if h == nil {
		return HarnessSeedResult{}, fmt.Errorf("harness receiver unavailable")
	}
	return h.seed(spec)
}
...
func (h *Harness) seed(spec HarnessSeedSpec) (result HarnessSeedResult, err error) {
	...
	if workflowSeed {
		...
		if h.config.Host == nil {
			return HarnessSeedResult{}, fmt.Errorf("seed workflows: harness host unavailable")
		}
		if requireErr := h.config.Host.RequireWorkflowEngine(); requireErr != nil {
...
func (h *Harness) seedHomeFile(file HarnessSeedHomeFile) (string, error) {
	...
	dest := filepath.Join(h.config.CredentialHome, filepath.FromSlash(rel))
...
func (h *Harness) seedProject(spec HarnessSeedProject) (HarnessSeedProjectResult, error) {
	...
	if spec.Repo != nil {
		...
		path = filepath.Join(h.config.DataRoot, "workspaces", spec.Name)
	...
	database := h.store()
	if h.config.Host == nil || database == nil {
		return HarnessSeedProjectResult{}, fmt.Errorf("harness host or store unavailable")
	}
	projectRow, err := h.config.Host.CreateProject(path)
	if err != nil {
	...
	projectRow, err = database.GetProject(projectRow.ID)
	if err != nil {
...
func (h *Harness) seedThread(projectID string, spec HarnessSeedThread) (string, error) {
	...
	thread, err := h.config.Host.CreateThread(ThreadOptions{
		ProjectID:   projectID,
		Title:       spec.Title,
	...
	if spec.SessionRef != "" {
		if _, err := h.store().UpdateSessionRef(thread.ID, spec.SessionRef); err != nil {
...
func (h *Harness) HarnessReset() (err error) {
	...
	database := h.store()
	if h.config.Host == nil || database == nil {
		return fmt.Errorf("harness host or store unavailable")
	}
	threads, err := database.ListThreads()
	if err != nil {
		return fmt.Errorf("list threads: %w", err)
	}
	for _, t := range threads {
		if err := h.config.Host.StopSession(t.ID); err != nil {
	...
	if err := h.config.Host.RecoverCrashedTurns(); err != nil {
		return fmt.Errorf("settle in-flight turns: %w", err)
	}
	// StopSession made every persisted running background launch an orphan.
	// Production deletion correctly refuses those rows, so settle them through
	// the same idempotent path startup uses before applying the delete cascade.
	if err := h.config.Host.RecoverOrphanedBackgroundTasks(); err != nil {
	...
	projects, err := database.ListProjectsWithThreadCounts()
	if err != nil {
	...
	for _, p := range projects {
		...
		if err := database.DeleteProjectWorkflowRecords(p.Project.ID); err != nil {
			return fmt.Errorf("delete workflow records for project %s: %w", p.Project.ID, err)
		}
		if err := h.config.Host.DeleteProject(p.Project.ID); err != nil {
	...
	if err := os.RemoveAll(filepath.Join(h.config.DataRoot, "workspaces")); err != nil {
		return fmt.Errorf("remove generated workspaces: %w", err)
	}
	for _, dir := range []string{
		filepath.Join(h.config.DataDir, "projects"),
		filepath.Join(h.config.DataDir, "workflows"),
		filepath.Join(h.config.DataDir, "workflow-runs"),
	} {
	...
	if h.config.CredentialHome != "" {
		for _, name := range []string{".claude", ".claude.json", ".codex"} {
			p := filepath.Join(h.config.CredentialHome, name)
