...
// or a trim RPC a LAN peer can drive.
func TestWebviewTrimContractClassifications(t *testing.T) {
	...
	if !LocalOnlyMethods["RequestWebviewMemoryTrim"] {
		t.Error(`"RequestWebviewMemoryTrim" must be LocalOnly: a remote peer's idleness says nothing about the desktop session, and the method is a jank lever`)
	}
}
