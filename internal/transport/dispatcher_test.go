...
// TestDispatcher_ReadyGate_DefaultOpen pins the back-compat contract:
// a fresh dispatcher serves traffic without any explicit SignalReady
// call so existing tests and the clientmode stub don't have to opt in.
func TestDispatcher_ReadyGate_DefaultOpen(t *testing.T) {
	d, _ := newTestDispatcher(t, RegisterOptions{Package: "main", TypeName: "App"})
	m, _ := d.Resolve(0, "Greet")
	done := make(chan struct{})
	go func() {
		defer close(done)
		_, fe := d.Invoke(context.Background(), m, []json.RawMessage{json.RawMessage(`"world"`)})
		if fe != nil {
			t.Errorf("invoke on default-ready dispatcher: %v", fe)
		}
	}()
	select {
	case <-done:
	case <-time.After(time.Second):
		t.Fatalf("Invoke blocked on a dispatcher that was never HoldUntilReady'd")
	}
}

// TestDispatcher_ReadyGate_HoldUntilReadyParksInvoke confirms the boot
// contract: after HoldUntilReady, Invoke blocks rather than letting
// the method body run. SignalReady releases the wait.
func TestDispatcher_ReadyGate_HoldUntilReadyParksInvoke(t *testing.T) {
	d, app := newTestDispatcher(t, RegisterOptions{Package: "main", TypeName: "App"})
	d.HoldUntilReady()

	m, _ := d.Resolve(0, "Greet")
	done := make(chan *FrameError, 1)
	go func() {
		_, fe := d.Invoke(context.Background(), m, []json.RawMessage{json.RawMessage(`"world"`)})
		done <- fe
	}()

	// Give the goroutine a moment to reach the wait. The body must not
	// fire yet — the receiver records every call, so a non-empty calls
	// slice proves the gate was bypassed.
	select {
	case fe := <-done:
		t.Fatalf("Invoke returned %v before SignalReady; gate did not hold", fe)
	case <-time.After(50 * time.Millisecond):
	}
	app.mu.Lock()
	if len(app.calls) != 0 {
		t.Fatalf("method body executed before SignalReady: %v", app.calls)
	}
	app.mu.Unlock()
	...
	case fe := <-done:
		if fe != nil {
			t.Fatalf("Invoke after SignalReady: %v", fe)
		}
	case <-time.After(time.Second):
		t.Fatalf("Invoke did not return after SignalReady")
	}
}

// TestDispatcher_ReadyGate_CtxCancelReleasesWaiter mirrors the
// connection-shutdown path: a ctx-cancel while the dispatcher is
// holding returns the generic internal frame rather than parking
// forever.
func TestDispatcher_ReadyGate_CtxCancelReleasesWaiter(t *testing.T) {
	d, _ := newTestDispatcher(t, RegisterOptions{Package: "main", TypeName: "App"})
	d.HoldUntilReady()
	...
	go func() {
		_, fe := d.InvokeForOrigin(ctx, m, []json.RawMessage{json.RawMessage(`"world"`)}, true)
		done <- fe
	}()

	// Confirm the wait is active before cancelling.
	select {
	case fe := <-done:
		t.Fatalf("Invoke returned %v before ctx cancel; gate did not hold", fe)
	case <-time.After(50 * time.Millisecond):
	}
	...
	case fe := <-done:
		if fe == nil {
			t.Fatalf("Invoke returned nil after ctx cancel; expected internal frame error")
		}
		if fe.Code != ErrCodeInternal {
			t.Fatalf("ctx-cancel error code = %s, want %s", fe.Code, ErrCodeInternal)
		}
	case <-time.After(time.Second):
		t.Fatalf("Invoke did not return after ctx cancel")
	}
}

// TestDispatcher_ReadyGate_SignalReadyIdempotent — closing a closed
// channel would panic; pin that a repeat SignalReady is a no-op.
func TestDispatcher_ReadyGate_SignalReadyIdempotent(t *testing.T) {
	d, _ := newTestDispatcher(t, RegisterOptions{Package: "main", TypeName: "App"})
	d.HoldUntilReady()
	d.SignalReady()
	d.SignalReady()

	// Sanity: Invoke still works after the double-signal.
	m, _ := d.Resolve(0, "Greet")
	if _, fe := d.Invoke(context.Background(), m, []json.RawMessage{json.RawMessage(`"x"`)}); fe != nil {
		t.Fatalf("post-double-signal invoke: %v", fe)
	}
}

// TestDispatcher_ReadyGate_MultipleWaitersRelease pins fan-out: a
// real boot window can have several WS connections queue RPCs before
// SignalReady fires. They must all release on the single close.
func TestDispatcher_ReadyGate_MultipleWaitersRelease(t *testing.T) {
	d, _ := newTestDispatcher(t, RegisterOptions{Package: "main", TypeName: "App"})
	d.HoldUntilReady()

	m, _ := d.Resolve(0, "Greet")
	const waiters = 8
	done := make(chan *FrameError, waiters)
	for i := 0; i < waiters; i++ {
		go func() {
			_, fe := d.Invoke(context.Background(), m, []json.RawMessage{json.RawMessage(`"w"`)})
			done <- fe
		}()
	}

	// All waiters should still be parked before SignalReady.
	select {
	case fe := <-done:
		t.Fatalf("waiter returned %v before SignalReady", fe)
	case <-time.After(50 * time.Millisecond):
	}
	...
	for i := 0; i < waiters; i++ {
		...
		case fe := <-done:
			if fe != nil {
				t.Fatalf("waiter %d returned error: %v", i, fe)
			}
		case <-timeout:
			t.Fatalf("only %d/%d waiters released", i, waiters)
		}
	}
}

// TestDispatcher_ReadyGate_HoldAfterSignalResetsGate pins that
// HoldUntilReady installs a fresh gate even after SignalReady — a
// closed channel can't be reopened, so this property depends on the
// install replacing the channel rather than reusing it.
func TestDispatcher_ReadyGate_HoldAfterSignalResetsGate(t *testing.T) {
	d, _ := newTestDispatcher(t, RegisterOptions{Package: "main", TypeName: "App"})
	d.HoldUntilReady()
	d.SignalReady()
	d.HoldUntilReady() // back to "not ready"

	m, _ := d.Resolve(0, "Greet")
	done := make(chan *FrameError, 1)
	go func() {
		_, fe := d.Invoke(context.Background(), m, []json.RawMessage{json.RawMessage(`"x"`)})
		done <- fe
	}()
	select {
	case fe := <-done:
		t.Fatalf("Invoke returned %v; gate did not re-hold after reset", fe)
	case <-time.After(50 * time.Millisecond):
	}
	...
	case fe := <-done:
		if fe != nil {
			t.Fatalf("post-reset invoke: %v", fe)
		}
	case <-time.After(time.Second):
		t.Fatalf("Invoke did not return after second SignalReady")
	}
}
