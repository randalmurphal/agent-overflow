...
// TestIntegration_ReadyGateHoldsRPCsUntilSignal pins the boot-window
// behaviour end-to-end across the wire: a real WS client sends an RPC
// frame while the dispatcher is gated, the response sits unsent until
// Server.SignalReady fires, and once released the response arrives
// with the receiver-side method body having executed exactly once.
//
// Without the gate, runHeadless can't safely publish /bootstrap.json
// before App.ServiceStartup completes — the SPA's first RPC would hit
// a half-initialised store. This test exists so a future refactor
// that "simplifies" the dispatcher by dropping the gate gets caught
// instead of silently re-introducing the boot-order race.
func TestIntegration_ReadyGateHoldsRPCsUntilSignal(t *testing.T) {
	f := newIntegrationFixture(t)
	// Flip the gate AFTER the fixture has booted so the boot itself
	// runs unblocked. Production calls HoldUntilReady inside
	// bootTransport, before Start; here the in-test ordering doesn't
	// matter because no client has dialled yet.
	f.srv.cfg.Dispatcher.HoldUntilReady()
	...
	frame := ClientFrame{
		Type:     frameTypeRPC,
		ID:       "gated-greet",
		MethodID: fnvHash("main.App.SimpleCall"),
		Params:   []json.RawMessage{json.RawMessage(`"gated"`)},
	}
	buf, err := json.Marshal(frame)
	if err != nil {
		t.Fatalf("marshal frame: %v", err)
	...
	// Spin a reader on a goroutine so we can prove the response is
	// pending (no frame in the next ~100ms) before SignalReady fires.
	readCtx, readCancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer readCancel()
	respCh := make(chan ServerFrame, 1)
	...
	select {
	case resp := <-respCh:
		t.Fatalf("received response %+v before SignalReady; gate did not hold", resp)
	case err := <-readErrCh:
		t.Fatalf("ws read failed before SignalReady: %v", err)
	...
	case resp := <-respCh:
		if resp.Error != nil {
			t.Fatalf("gated RPC returned error after SignalReady: %+v", resp.Error)
		}
		if string(resp.Result) != `"hello gated"` {
			t.Fatalf("gated RPC result = %s, want \"hello gated\"", string(resp.Result))
		}
	case err := <-readErrCh:
		t.Fatalf("ws read failed after SignalReady: %v", err)
	case <-time.After(2 * time.Second):
		t.Fatalf("no response after SignalReady; gate did not release")
	}
}
