...
// longer loopback-only/ephemeral, or an install-status method that stopped
// being host-scoped.
func TestSelfUpdateContractClassifications(t *testing.T) {
	...
	if got := classify(selfupdate.RPCReportStatus).Scope; got != ScopeHost {
		t.Errorf("%q carries scope %q, want %q: it settles install state, clears an on-disk marker, and releases the updater busy fence — acts on the host, with no remote form",
			selfupdate.RPCReportStatus, got, ScopeHost)
	}
}
