...
| `EventCommandResult` | `command_result` row, with the full text in a payload above the inline bound. `CommandResultMeta.AgentResult` is preserved so a recovered review/skill answer keeps its sourced-result presentation. |
