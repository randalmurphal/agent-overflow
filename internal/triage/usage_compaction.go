...
// describe the main agent, stay exactly where they are. The live mirror
// (claude parse_transcript_mirror.go) delivers the boundary at its real
// position, keyed by the transcript row's provider uuid.
