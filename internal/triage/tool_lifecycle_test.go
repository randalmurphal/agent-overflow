...
func TestToolStartPersistsLifecycleItem(t *testing.T) {
	...
	scopedID := providerScopedItemID("t1", "tool-1")
...
func TestToolCompleteFlipsInlineStatus(t *testing.T) {
	...
	for _, item := range upserted {
		if item.ID != providerScopedItemID("t1", "tool-2") {
			t.Errorf("upsert id %q, want %q", item.ID, providerScopedItemID("t1", "tool-2"))
		}
	}
}
...
func TestToolCompleteOnBackgroundedAppendsCompletion(t *testing.T) {
	...
	if done.CompletionOf != providerScopedItemID("t1", "bg-tool") {
		t.Errorf("CompletionOf = %q, want %q", done.CompletionOf, providerScopedItemID("t1", "bg-tool"))
	}
...
func TestInlineCompletionPreservesRichPayload(t *testing.T) {
	...
	preCompletion, _, _ := st.GetItem(providerScopedItemID("t1", "fc-rich"))
	...
	postCompletion, _, _ := st.GetItem(providerScopedItemID("t1", "fc-rich"))
...
func TestInlineCompletionAttachesPayloadWhenNoneExists(t *testing.T) {
	...
	item, _, _ := st.GetItem(providerScopedItemID("t1", "bash-1"))
	if item.PayloadID == "" {
