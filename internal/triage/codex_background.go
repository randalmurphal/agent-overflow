...
//      reports at least one child thread in a non-terminal state at the
//      time the parent's turn closes.
//
// The projector sits between the wire signal and the is_background stamp.
// It tracks per-thread inProgress unifiedExec items and their launching
// turn, then flips the row on the first MODEL-PRODUCED event following
// the spawn (text delta, reasoning delta, or turn-completed as the
// catchall). A yield moment is the unambiguous "the agent has moved on
// while this command is still running" signal — i.e. this is no longer
// a synchronous tool call.
//
// The turn-level correlation is bounded: state clears on turn/completed
// (stamp + drop) and on CleanupThread (session teardown drop). Nothing
...
// the projector between item/started and item/completed. turnIndex is
// the store's turn index at spawn time (retained for potential
// tray-enrichment use; the projector's own logic pulls the current
// turn index at sibling-synthesis time so it lands at the timeline
// tail). backgrounded is set once a yield has been observed and the
// row has been stamped is_background=true — prevents double-emission.
...
// the wire but its child thread may outlive the parent's turn. The
// projector stamps is_background=true at the parent's turn/completed if
// any child is still in a non-terminal state at that point.
...
//     turn-complete).
//   - collabAgentToolCall spawn_agent: tracked; stamped on turn/completed
//     if agentsStates reports a running child. The spawn row itself
//     closes on the wire immediately — observeCodexToolComplete refreshes
//     the running-children flag from the end envelope.
//
...
func (r *Router) observeCodexToolStart(evt provider.ProviderEvent) {
	...
	if meta.Source != "unifiedExecStartup" && evt.ItemType != "collab_agent" {
		return
	}

	turnIndex, err := r.currentTurnIndex(evt.ThreadID)
	if err != nil {
		log.Printf("triage: codex-background turn index on tool start %s: %v", itemID, err)
		return
	}
	...
	case meta.Source == "unifiedExecStartup":
		...
		// the flag or clobber the turn correlation.
	...
	case evt.ItemType == "collab_agent":
		...
		// turn correlation so the later refresh can resolve it.
		if _, ok := state.spawnAgent[itemID]; ok {
			return
		}
		state.spawnAgent[itemID] = &spawnAgentTracker{turnIndex: turnIndex}
	}
}
...
func (r *Router) observeCodexToolComplete(evt provider.ProviderEvent) error {
	...
	if unifiedTracker != nil {
		...
		// here keeps the inProgress map bounded.
		r.mu.Lock()
		delete(r.codexBackground[evt.ThreadID].unifiedExec, itemID)
		r.mu.Unlock()
	...
	if spawnTracker != nil {
		meta := decodeCodexItemMeta(evt.Meta)
		running := collectRunningChildren(meta.AgentsStates)
		r.mu.Lock()
		spawnTracker.hasRunningChildren = len(running) > 0
		...
		// can decide whether to stamp is_background.
		if !spawnTracker.hasRunningChildren {
			delete(r.codexBackground[evt.ThreadID].spawnAgent, itemID)
		}
		r.mu.Unlock()
	}
...
// sibling synthesis reads for status / outcome derivation.
...
// EventThinking. Any tracked unifiedExec items that haven't already been
// stamped get flipped to is_background=true and re-emitted so the tray
...
// while the command is still running. EventToolStart explicitly does
// NOT participate in yield detection — sibling tool starts in a parallel
// batch fire before any model text.
func (r *Router) observeCodexModelYield(threadID string) {
	r.mu.Lock()
	state := r.codexBackground[threadID]
	if state == nil || len(state.unifiedExec) == 0 {
		r.mu.Unlock()
		return
	}
	toFlip := make([]string, 0, len(state.unifiedExec))
...
// observeCodexTurnComplete is the catchall. Runs after the tool
// lifecycle's force-close pass (invariant 23) but with is_background=true
// exemption from that pass — any unifiedExec or spawn_agent item that
// remained inProgress / has running children through the turn boundary
// gets stamped backgrounded here.
...
// thread id) and synthesizes a sibling completion row for any
// is_background=true spawn_agent rows that pointed at the now-closed
// child. Persistence failures on any single sibling return up so the
// calling handleSubagentNotification surfaces them — we log per
// failure but the first error still escapes rather than silently
// leaving the tray stale.
func (r *Router) observeCodexSubagentNotification(threadID string, meta json.RawMessage) error {
	parsed := decodeSubagentNotificationMeta(meta)
	if parsed.AgentPath == "" {
		return nil
	}
...
func (r *Router) synthesizeCodexBackgroundCompletion(evt provider.ProviderEvent, launchID string) error {
	...
	tailTurn, err := r.currentTurnIndex(evt.ThreadID)
	if err != nil {
		// Fall back to the launch turn rather than drop the row. A
		// missing open-turn map entry means no turn is in flight —
		// legitimate at session-end; the launching turn is still a
		// valid home for the sibling in that narrow case.
		tailTurn = launch.TurnIndex
	}
...
// collectRunningChildren extracts the list of child thread ids whose
// entry in agentsStates is a non-terminal state. The agentsStates map
// keys are child thread ids; values are CollabAgentStatus variants
// ("pendingInit" | "running" | "interrupted" | "completed" | "errored"
// | "shutdown" | "notFound"). Running or pendingInit count as
// non-terminal; anything else is terminal. The value may be either a
// bare string ("running") or an object ({status: "running"}) — codex
// ships both shapes depending on wire version.
func collectRunningChildren(states map[string]json.RawMessage) []string {
