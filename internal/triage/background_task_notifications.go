...
func (r *Router) handleBackgroundTaskNotification(evt provider.ProviderEvent) error {
	...
	case !isSubagentTranscriptLaunch(launch):
		...
		payload, readErr := backgroundOutputPayload(launch, meta.OutputFile, "", nil, now)
		if readErr != nil {
			outputState = "error"
...
// stop: the live background commands at its transcript root it waits on
// (zero for a final stop) and that root, where the round's report lives.
type parkedStop struct {
	waiting int
	rootID  string
}
...
// (agentReportFromNotification); a notification without one leaves the
