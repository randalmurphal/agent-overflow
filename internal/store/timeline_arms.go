...
// on the lineage row `l`, or "") selects. The hand-off and copy-on-write
// reads use it, so they find inherited rows through the same sources and
// visibility rule as every timeline read.
...
// name the lineage row `l`. The ancestor's rows are found through the
// payload keys, never through the ancestor's whole thread.
func inheritedPayloadRowArms(columns, levels string) string {
	return `SELECT ` + columns + `
		  FROM thread_fork_lineage l
		  CROSS JOIN items ON items.thread_id = l.ancestor_id
...
// the rows that reference it (shadowInheritedPayloadTx, handOffIDsTx).
