...
func TestSyncThreadWindowStatuses(t *testing.T) {
	...
	got, err := s.SyncThreadWindow(ctx, "t", "", 200, held, nil)
	if err != nil {
	...
	got, err = s.SyncThreadWindow(ctx, "t", "", 200, UnknownHistoryStamp(), nil)
	if err != nil {
	...
	// gone — no thread row.
	got, err = s.SyncThreadWindow(ctx, "no-such-thread", "", 200, held, nil)
	if err != nil {
...
func TestSyncThreadWindowHonorsAnchor(t *testing.T) {
	...
	got, err := s.SyncThreadWindow(context.Background(), "t", "t-i2", 4, UnknownHistoryStamp(), nil)
	if err != nil {
		t.Fatalf("sync: %v", err)
	}
	if got.Page == nil {
...
func TestRestoreFromRemintsReplicaGeneration(t *testing.T) {
	...
	got, err := st.SyncThreadWindow(context.Background(), "t1", "", 200, UnknownHistoryStamp(), nil)
	if err != nil {
...
func TestSyncThreadWindowZeroStampMatchesOnlyAnUntouchedThread(t *testing.T) {
	...
	zero := HistoryStamp{}
	got, err := s.SyncThreadWindow(ctx, "with-history", "", 200, zero, nil)
	if err != nil {
	...
	got, err = s.SyncThreadWindow(ctx, "untouched", "", 200, zero, nil)
	if err != nil {
...
func TestSyncThreadWindowAttestsUnderConcurrentWrites(t *testing.T) {
	...
	for {
		sync, err := s.SyncThreadWindow(context.Background(), "t", "", 200, UnknownHistoryStamp(), nil)
		if err != nil {
		...
		case err := <-writerDone:
			...
			final, err := s.SyncThreadWindow(context.Background(), "t", "", 200, UnknownHistoryStamp(), nil)
			if err != nil {
