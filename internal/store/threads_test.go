...
var threadColumnsNotWrittenByUpdateThread = map[string]string{
	...
	"fork_source_thread_id":    "the pointer fork's source (v119), written by fork creation and cleared by source deletion or materialization; a stale struct must not relink or unlink history",
	"fork_cut_turn_index":      "the pointer fork's cut (v119), owned with its lineage rows by fork creation and a fork's own revert",
	"fork_cut_item_index":      "the other half of that cut (v119), same writers",
	"fork_source_title":        "the divider's source title (v119), recorded at fork creation and when the source is deleted",
}
...
func TestThreadReadStateQueryWalksTheItemIndex(t *testing.T) {
	...
	query, args, err := threadReadStateQuery(s.db, threadID)
	if err != nil {
		t.Fatal(err)
	}
