...
func TestSubagentAggregateStatementPlans(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{"card row adopting its children", itemInsertAdoptingSQL, "SEARCH agg_hc USING INDEX idx_items_parent (thread_id=? AND parent_id=?)",
			itemInsertArgs(child), boundedPlan{}},
		...
		{"flush of a carrier's first stamp", flushSubagentCarrierInsertSQL, "SEARCH agg_hc USING INDEX idx_items_parent (thread_id=? AND parent_id=?)",
