...
// scratchThreadsV101SQL admits the `scratch` thread mode and adds the table
// that records what a scratch fork came from.
//
// The mode lives in the `threads` CHECK constraint, so it needs the full
// table rebuild v72 established. The SQL below restates the CURRENT table
// shape (v72's columns plus the ALTER-added group_id, created_by_device,
// created_branch, created_remote_url and created_head_commit) instead of
// deriving from v72's text: a new rebuild migration must not make shipped
// migration SQL live source (see migrate.go).
//
// Everything a rebuild must carry over comes back at the end: the six
// indexes, the v100 generation of the history triggers, the imported-history
// guard trigger, the two thread_import_state uniqueness triggers, and the
// owned_threads view, which selects `threads.*` and therefore has to be
// dropped before the table it names goes away.
var scratchThreadsV101SQL = dropHistoryRevTriggersSQL + `
...
DROP VIEW IF EXISTS owned_threads;
...
CREATE TABLE threads_new (
    ...
    mode                     TEXT    NOT NULL DEFAULT 'chat'
        CHECK(mode IN ('chat','plan','discussion','terminal','workflow','workflow-studio','workflow-triage','scratch')),
...
INSERT INTO threads_new (
    ...
    created_branch, created_remote_url, created_head_commit
)
SELECT
    id, project_id, title, provider, model, workspace_path, worktree_path,
    branch, pr_ref, session_ref, pending_fork_session_ref, mode, reasoning_effort,
...
CREATE TABLE scratch_threads (
    thread_id        TEXT PRIMARY KEY REFERENCES threads(id) ON DELETE CASCADE,
    source_thread_id TEXT NOT NULL,
    return_mode      TEXT NOT NULL
        CHECK(return_mode IN ('chat','plan','discussion','terminal','workflow','workflow-studio','workflow-triage')),
    created_at       INTEGER NOT NULL,
    request_token    TEXT
);
