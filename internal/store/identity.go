...
func (s Session) Live(now int64) bool {
	return s.ActivatedAt != 0 && s.RevokedAt == 0 && s.DeviceRevokedAt == 0 && !s.Expired(now)
}

// ProcessBound reports whether the session uses the local boot credential.
func (s Session) ProcessBound() bool { return s.BindingClass == "loopback-only" && s.ExpiresAt == 0 }

// Expired checks timed access. Only an explicit local lifetime has no deadline.
func (s Session) Expired(now int64) bool { return !s.ProcessBound() && s.ExpiresAt <= now }
