...
// NormalizeSystemPath removes macOS's root-owned /var and /tmp aliases without
// resolving any caller-controlled symlink below them. Safety validators can
// then keep rejecting symlinked run/capture components while accepting the
// ordinary paths os.TempDir and t.TempDir return on macOS.
func NormalizeSystemPath(path string) string {
	clean := filepath.Clean(path)
	if runtime.GOOS != "darwin" || !filepath.IsAbs(clean) {
		return clean
	}
	for _, alias := range []string{"/var", "/tmp"} {
		rel, err := filepath.Rel(alias, clean)
		if err != nil || rel == ".." || strings.HasPrefix(rel, ".."+string(filepath.Separator)) {
			continue
		}
		resolved, err := filepath.EvalSymlinks(alias)
		if err != nil || filepath.Clean(resolved) == alias {
			return clean
		}
