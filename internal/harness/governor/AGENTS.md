...
**It is not the OOM protection.** `internal/harness/containment` installs
the kernel boundary (cgroup v2, Job Object, `RLIMIT_DATA`) and that is
what stops a run. This package is cross-process accounting plus a
diagnostic host-floor backstop. Do not add a kill path here.
