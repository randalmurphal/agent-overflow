...
func (darwinProcesses) RSS(pid int) (uint64, error) {
	// Darwin ps accepts one comma-separated format after -o. Passing each
	// column as a separate argv (accepted by some Linux implementations)
	// fails with "illegal argument: ppid=" on macOS.
