...
| `procrss/` | Per-process RSS for THIS process and the webview children it spawned, read straight out of `/proc/<pid>/status`. Backs the backend half of a harness perf run (`app_harness_perf.go`), where "how big did the renderer get" is the number under test and gopsutil's whole-host view is the wrong shape. `SampleRoot` takes the proc root so the walk is testable against a canned tree; matching is by name PREFIX because the kernel truncates `Name:` at 15 chars. Linux-only behind a `_linux` build split; everywhere else `Sample` returns `ErrUnsupported` and the sampler records an absent series rather than failing. Stdlib-only. |
