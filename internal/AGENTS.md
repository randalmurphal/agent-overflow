...
| `procrss/` | Per-process RSS for THIS process and the webview children it spawned, read out of a `/proc`-shaped tree in two passes — `stat` (one line: comm + ppid) for every pid, because the parent map needs them all, then `status` (VmRSS) only for the processes that turned out to be ours. Backs the backend half of a harness perf run (`app_harness_perf.go`), where "how big did the renderer get" is the number under test and gopsutil's whole-host view is the wrong shape. `SampleRoot` takes the proc root so the walk is testable against a canned tree; matching is by name PREFIX because the kernel truncates `Name:` at 15 chars. Linux-only behind a `_linux` build split; everywhere else `Sample` returns `ErrUnsupported` and the sampler records an absent series rather than failing. Stdlib-only. |
...
| `harness/` | Agent test harness engines behind the `--harness` boot mode: git-repo fixtures + wire-level event replay, `control/` (loopback control channel between the harness and `ao-mockprovider` processes), `scenario/` (mock scenario schema + embedded library), `instanceinfo/` (instance id + the token-free registry a tool discovers running harness/soak instances through). Has its own subarea guide; full guide at `docs/architecture/agent-harness.md`. |
| `harnessclient/` | The Go client for a running harness or soak instance, and the twin of `e2e/src/harness.ts`: bootstrap discovery (the `<dataDir>/harness-instance.json` file, or the `__AO_HARNESS__` line a spawned backend prints), the WebSocket peer speaking `internal/transport`'s frames, detached launch, and file tailing. It restates the frame shapes rather than importing them, so `cmd/ao-harness` links no server code; the tests decode this package's frames through the real `transport` structs, which is what keeps the restatement from drifting for free. `WaitForEvent` consumes its match exactly as the TS client does, so two identical waits observe two occurrences. Has its own subarea guide. |
| `harnessrun/` | Durable harness run manifests, ownership leases, process-group teardown, artifact capture, and host-global failed-run quarantine retention. Retention is explicit at the supervisor boundary so tests and custom harnesses use an isolated registry. |
| `cdpclient/` | Minimal Chrome DevTools Protocol client: target discovery over the debugger's HTTP endpoint, page selection that never guesses between tabs, one WebSocket with id correlation, and a subscription channel for domain events. Backs `ao-harness profile` (V8 sampling profiler) and `ao-harness bench --trace` (timeline trace), the two harness verbs with no engine-agnostic equivalent — a WebKitGTK instance serves no CDP at all. Deliberately NOT `chromedp/cdproto` (already in the module): six call sites do not justify linking generated bindings for every domain into the harness CLI, so typed helpers live beside their callers and this stays about the wire. The 256 MiB read limit is load-bearing — `Profiler.stop` answers with the whole profile in one frame. Has its own subarea guide. |
...
| `eventchan/` | `type Channel string` plus one constant per event channel the backend pushes. Imports NOTHING (not even from this repo) so every emitting layer — `main`, `transport`, `triage`, `workflow/engine`, `screenshot` — can take it in its emit signature. It is the SPELLING half of one table whose POLICY half is `transport`'s `channelPolicies`; a cross-check test AST-parses this package and fails on either half missing its counterpart, so adding a channel is two edits and no more. The newtype stops a channel *variable* crossing without an explicit conversion (which is what the harness escape hatches spell); the root package's `TestEmitSitesNameAnEventChannelConstant` stops a bare string LITERAL, which Go would otherwise assign silently. Has its own subarea guide. |
...
| `threadtitle/` | Pure prompt builders (first-turn + regeneration), the newest-first thread-context formatter behind regeneration, schema constants, decoder, and sanitiser behind the thread-title flow. Workspace resolution, image-attachment plumbing, the store read feeding the context, and the compare-and-swap into `store.UpdateTitleIfCurrent` stay in the main package; CLI-error redaction is `textgen.RedactError`. |
...
| `providerschema/` | The strict-mode rules both provider CLIs enforce on structured-output schemas (`Validate` → `[]Violation`), each backed by an observed CLI rejection. `ValidateClaude` is the same walk minus the two codex-only object rules, for a schema that is Claude's ALONE (`commitmsg` / `threadtitle` each keep one) — never for a schema sent to both. Stdlib-only and JSON-in, so schema generation (`workflow/def`, test-only) and schema consumption (`cmd/ao-mockprovider`) share one definition of "a provider will accept this". |
...
| `workflowhost/` | The app-side workflow RUNNER: the `engine.Runner` implementation that turns one phase, unit, or join into a live provider turn or a supervised command, and the per-attempt machinery around it — workspace provisioning (item worktree, fan-out sub-worktrees), reliability timers, the send epoch ladder, the start watchdog, the envelope/turn observer, human takeover, the typed usage-limit park, narratives, and artifact capture. It reaches the process around it ONLY through `Host` (`host.go`): eight capability-named consumer-side interfaces, satisfied in `main` by the pure-glue `workflowHostAdapter` (`app_workflow_host.go`). Bound methods stay in `main` — this package registers nothing on the wire. Has its own subarea guide. |
