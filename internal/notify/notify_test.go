...
func TestValidateTarget(t *testing.T) {
	tests := []struct {
		...
		{name: "item with thread", target: Target{Kind: "workflow-item", WorkItemID: "item", ThreadID: "thread"}},
		{name: "oversized item id", target: Target{Kind: "workflow-item", WorkItemID: strings.Repeat("x", MaxWorkItemIDBytes+1)}},
		{name: "removed triage-agent kind", target: Target{Kind: "workflow-triage-agent"}},
	}
