...
func TestNewCredentialsWithFileKeychainInstallsTheStandIn(t *testing.T) {
	credentials, err := NewCredentialsWithFileKeychain(t.TempDir(), Policy{})
	if err != nil {
		t.Fatal(err)
	}
	if _, ok := credentials.keychain.(fileClaudeKeychain); !ok {
...
func TestFileKeychainStateFollowsTheHomeNotTheInstance(t *testing.T) {
	...
	first, err := NewCredentials(sharedHome, Policy{})
	if err != nil {
		t.Fatal(err)
	}
	second, err := NewCredentials(sharedHome, Policy{})
	if err != nil {
		t.Fatal(err)
	}
	...
	other, err := NewCredentials(t.TempDir(), Policy{})
	if err != nil {
		t.Fatal(err)
	}
