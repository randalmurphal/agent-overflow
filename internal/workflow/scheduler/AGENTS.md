...
- `trigger` — `{kind, fired-at, scheduled-for?, event?{kind, item-id,
  workflow-id, state, reason}}`.
- `job-notes` — the automation's continuity notes, verbatim. The point
...
- `app_workflow_automations.go` — the RPC surface and its validation.
- `app_workflow_scheduler.go` — lifecycle, the start callback, and the
  event queue.
