...
func TestCallPhaseRefusesFieldsThatConfigureWork(t *testing.T) {
	cases := []struct {
		...
		{"capabilities", func(p *Phase) { p.Capabilities = []string{"start-run"}; p.MCP = []string{"srv"} }, "capabilities/mcp/grants is not valid on a call phase"},
		{"grants", func(p *Phase) { p.Grants = []string{string(GrantStartRun)} }, "capabilities/mcp/grants is not valid on a call phase"},
