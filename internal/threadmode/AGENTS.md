...
- **Interaction mode** — `chat`, `plan`, `design` (or `discussion`, which
  is set only through `StartDiscussion`). Controls the UX shell and how
...
| Function | Purpose |
|---|---|
| `ValidateCreate(mode) (string, error)` | Normalises an interaction mode for `CreateThread`. Empty → `DefaultCreateMode` ("chat"). Accepts `chat`/`plan`/`design`. Rejects `discussion` (must go through `StartDiscussion`) and unknown values. |
| `ValidateSet(mode) (string, error)` | Normalises an interaction mode for `UpdateThreadMode`. Accepts `chat`/`plan` only — `design` and `discussion` are immutable thread *types* set at creation. Rejects empty (UpdateThreadMode is always an explicit user action). |
...
- Discussion threads are intentionally not validated here. The
