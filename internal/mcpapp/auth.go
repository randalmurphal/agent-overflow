...
func (a *Service) triggerMcpAuth(threadID, name string) (MCPAuthInitResult, error) {
	if a.store == nil {
...
// SanitizeError bounds provider-originated MCP text before an adjacent
// app-owned MCP feature includes it in a user-facing wire error.
func SanitizeError(s string) string { return sanitizeMCPError(s) }

// claudeMCPOAuthPoll is the dedup identity stored in
// Service.claudeOAuthPolls. The struct address is the identity — a
...
func (a *Service) startClaudeMCPOAuthPoll(threadID, serverName string) {
	ctx, cancel := context.WithCancel(a.lifeCtx())
	poll := &claudeMCPOAuthPoll{cancel: cancel}

	a.claudeOAuthPollsMu.Lock()
	if a.claudeOAuthPolls == nil {
		a.claudeOAuthPolls = map[string]*claudeMCPOAuthPoll{}
	}
	if prior, ok := a.claudeOAuthPolls[serverName]; ok {
		prior.cancel()
	}
	a.claudeOAuthPolls[serverName] = poll
	a.claudeOAuthPollsMu.Unlock()
	...
	go func() {
		defer func() {
			...
			if a.claudeOAuthPolls[serverName] == poll {
				delete(a.claudeOAuthPolls, serverName)
			}
			a.claudeOAuthPollsMu.Unlock()
			cancel()
		}()
...
// DefaultClaudeMCPOAuthIntervals returns a copy of the production poll cadence.
func DefaultClaudeMCPOAuthIntervals() []time.Duration {
	return append([]time.Duration(nil), defaultClaudeMCPOAuthIntervals...)
}
...
func (a *Service) pollClaudeMCPAfterOAuth(
	ctx context.Context,
...
// PollClaudeMCPAfterOAuth exposes the observation loop for integration tests
// that verify root event and shutdown wiring with controlled provider replies.
func (a *Service) PollClaudeMCPAfterOAuth(
	ctx context.Context,
	threadID, serverName string,
	intervals []time.Duration,
	...
	a.pollClaudeMCPAfterOAuth(ctx, threadID, serverName, intervals, getQuerier)
}
...
func (a *Service) HandleCodexMCPOAuthCompleted(threadID, serverName string, success bool, errMsg string) {
...
// action.
func (a *Service) HandleCodexMCPStartupUpdate(u codex.MCPStartupUpdate) {
