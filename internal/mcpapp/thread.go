...
func (a *Service) ListThreadMcpServers(threadID string) ([]ThreadMCPServer, error) {
	if a.store == nil {
	...
	if sess, ok := a.session(threadID); ok {
...
// ignored.
func (a *Service) ListWorkspaceMcpServers(providerName, workspacePath string) ([]ThreadMCPServer, error) {
	switch providerName {
...
func (a *Service) SetThreadMcpServerEnabled(threadID, name string, enabled bool) error {
	if a.store == nil {
...
func (a *Service) SetWorkspaceMcpServerEnabled(providerName, workspacePath, name string, enabled bool) error {
	name = strings.TrimSpace(name)
	if name == "" {
...
func (a *Service) setClaudeThreadMCPEnabled(t store.Thread, name string, enabled bool) error {
	if sess, ok := a.session(t.ID); ok && sess.Claude != nil {
		ctx, cancel := context.WithTimeout(a.lifeCtx(), mcpLiveApplyTimeout)
		defer cancel()
...
func (a *Service) setClaudeWorkspaceMCPDisabled(workspacePath, name string, disabled bool) error {
...
func (a *Service) claudeConfigMCPRows(workspacePath string) ([]ThreadMCPServer, error) {
	st, err := a.claudeConfig()
	if err != nil {
		return nil, err
	}
...
func (a *Service) setCodexMCPEnabled(reloadThreadID, name string, enabled bool) error {
	...
	sess, ok := a.session(reloadThreadID)
...
// thread's Codex session. Present in Service.codexReloads exactly while a
...
func (a *Service) requestCodexMCPReload(threadID string) {
	...
	go func() {
		for {
			...
			st.rerun = false
			if !rerun {
				delete(a.codexReloads, threadID)
			}
			a.codexReloadsMu.Unlock()
...
// nothing to report.
func (a *Service) runCodexMCPReload(threadID string) error {
...
func (a *Service) codexSessionMCPRows(ctx context.Context, sess *codex.Session) ([]ThreadMCPServer, error) {
...
// focused integration tests without duplicating provider row-merging policy.
func (a *Service) CodexSessionMCPRows(ctx context.Context, sess *codex.Session) ([]ThreadMCPServer, error) {
	return a.codexSessionMCPRows(ctx, sess)
}

func (a *Service) codexConfigMCPRows() ([]ThreadMCPServer, error) {
	st, err := a.codexConfig()
	if err != nil {
		return nil, err
	}
...
func (a *Service) cachedMCPStatusByName(prov mcpstatus.Provider) map[string]mcpstatus.CachedStatus {
