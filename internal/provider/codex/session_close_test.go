...
var sessionCloseFieldDispositions = map[string]string{
	...
	"threadID": "identity",
	"workDir":  "identity",
	"developerInstructions": "bounded thread-level text composed once at start; a closed session's " +
		"value is the honest teardown answer, like turnConfig",
	"binary":         "identity",
	"nextID":         "request-id counter",
	"controlMu":      "root control-operation lock",
	"settingsWireMu": "settings and turn pipe-write ordering lock",
	"settingsSync":   "one joined worker, cleared pending flags, and a closed admission gate",
	"mu":             "the lock",
