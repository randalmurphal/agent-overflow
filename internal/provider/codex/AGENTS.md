...
| `account/login/start`, `account/login/cancel` | Sign-in only (`login.go`), on a process spawned for nothing else and holding an isolated `CODEX_HOME`. One login per process: switching variants CANCELS first. Every wait correlates by `loginId`, because a superseded or cancelled login emits a FAILED completion for its own id. `chatgptDeviceCode` is the only variant a person away from this machine can finish — the browser variant's URL completes on a loopback listener this child bound. |
