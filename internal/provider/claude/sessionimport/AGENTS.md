...
| subagent rows | same mappings with `ParentToolUseID` set | Emitted BETWEEN the Task launch and its completion. The first scoped user row carries `subagent_opening_prompt:true` and its transcript uuid in `ItemID`. The neutral writer keys it as `user:subagent-prompt:<launchID>`, matching the row live triage creates from the launch input before child output. Later scoped user rows keep provider-keyed identities. |
