...
// start attempts lives in app_session_runtime.go; thin wire façades live
// in app_session_bindings.go. Everything in this file
...
func (a *App) startSessionNowWithClaudeResumeAt(threadID, claudeResumeAt string) error {
	...
	// behind. See app_session_runtime.go.
...
func (a *App) workflowSchemaForSession(thread store.Thread) (json.RawMessage, error) {
	...
	schema, registered := a.workflowApplication().SessionSchemaForThread(thread.ID)
	if !registered {
