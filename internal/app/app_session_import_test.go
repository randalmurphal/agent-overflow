...
func wireImportScanClock(app *App, ttl time.Duration) (*time.Time, *int) {
	...
	scans := 0
	app.sessionImport.manager = app.newSessionImportManager(sessionimport.ManagerConfig{
		ScanTTL: ttl,
		Now:     func() time.Time { return now },
		Scan: func(ctx context.Context, deps sessionimport.Deps, _ sessionimport.Filter) (sessionimport.ScanResult, error) {
			scans++
...
func TestImportScanCacheLookupExpiresWithTheEntry(t *testing.T) {
	...
	*now = now.Add(2 * time.Minute)
	if *scans != 1 {
		t.Fatalf("scans = %d, want 1 before the expired id is resolved", *scans)
	}

	// A stale id is the run's cue to rescan, and the rescan re-mints it.
	frames := runImport(t, app, id)
	if frames[0].Status != sessionImportStatusImported {
		t.Fatalf("frame = %+v, want the expired id resolved by a rescan", frames[0])
	}
	if *scans != 2 {
...
func blockingScanCache(t *testing.T, app *App) (release func()) {
	...
	app.sessionImport.manager = app.newSessionImportManager(sessionimport.ManagerConfig{
		Scan: func(ctx context.Context, _ sessionimport.Deps, _ sessionimport.Filter) (sessionimport.ScanResult, error) {
			select {
			case <-gate:
