...
func registerLiveClaudeSession(t *testing.T, app *App, threadID string) {
	...
	app.sessionManager().put(threadID, session{Provider: string(provider.Claude), Token: "live-" + threadID})
}
...
func observeReconcileSteps(app *App) chan string {
	...
	app.sessionManager().runtime.SetReconcileSessionConfigStep(func(threadID string) { steps <- threadID })
	return steps
}
...
func TestLiveClaudeReconcileCoalescesSavesDuringASweep(t *testing.T) {
	...
	sweeps := 0
	app.sessionManager().runtime.SetReconcileSessionConfigStep(func(string) {
		sweeps++
		first := sweeps == 1
		entered <- struct{}{}
		if first {
	...
	for {
		running, dirty := app.sessionManager().runtime.LiveClaudeReconcileState()
		total := sweeps
		if !running {
			if dirty {
...
func TestSettingsSaveReachesTheWireOfASessionThatWasStillStarting(t *testing.T) {
	...
	if current.LaunchOptions.SystemPrompt != housePrompt {
		t.Fatalf("launchOpts.SystemPrompt = %q, want the applied override",
			current.LaunchOptions.SystemPrompt)
	}
}
