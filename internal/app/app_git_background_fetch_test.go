...
func memoKeys(app *App) []string { return app.gitApplication().BackgroundFetchErrorKeysForTesting() }
...
func TestBackgroundFetchPassFetchesEachRepositoryOnce(t *testing.T) {
	...
	want := "repo:" + commonDir
	if got := memoKeys(app); len(got) != 1 || got[0] != want {
...
func TestBackgroundGitFetchDisabledNeverStartsALoop(t *testing.T) {
	...
	if app.gitApplication().BackgroundFetchRunningForTesting() {
...
func TestBackgroundGitFetchStartIsIdempotentAndStops(t *testing.T) {
	...
	app.startBackgroundGitFetch()
	if !app.gitApplication().BackgroundFetchRunningForTesting() {
		t.Fatal("a second start stopped or replaced the running loop")
	}
