...
// The row is broadcast as `unlisted` so a second attached client drops it from
// its sidebar too; re-archiving an already-archived thread changes nothing and
// says nothing.
...
// without a follow-up GetThread round-trip, and broadcasts it as `listed` so
// every other attached client puts it back in its sidebar.
