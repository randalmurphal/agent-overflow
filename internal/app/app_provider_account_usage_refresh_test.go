...
func TestSelectedClaudeUsageRefreshRotatesInTheCanonicalHome(t *testing.T) {
	...
	active, err := providerCredentialsForTest(t, app).ReadCredential(string(provider.Claude), "", true)
	if err != nil {
		t.Fatal(err)
	}
...
func TestSelectedClaudeUsageRefreshWithoutRotationPublishesLimits(t *testing.T) {
	...
	fingerprint, blessed := app.providerAccounts.CredentialFingerprintForTest(string(provider.Claude))
...
func TestSelectedClaudeUsageRefreshPublishesLimitsWhenTheCommitFails(t *testing.T) {
	...
	app.rateLimitProbeClientOverride = usageClientWritingCredential(
		...
		"original",
		func() error {
			return providerCredentialsForTest(t, app).WriteNativeCredentialForTest(
				string(provider.Claude),
...
func TestUsageRefreshWithholdsLimitsWhenTheSelectionMoves(t *testing.T) {
	...
	<-started
	if _, err := app.providerAccounts.MetadataStoreForTest().Activate(string(provider.Claude), "other"); err != nil {
		t.Fatal(err)
	}
	close(release)
...
func TestManualUsageRefreshRefusesDuringBackoff(t *testing.T) {
	...
	app.providerLifecycleService().Backoff().Note(
		string(provider.Claude),
		"selected",
...
func TestManualUsageRefreshRecordsAPerAccountBackoff(t *testing.T) {
	...
	remaining := app.providerLifecycleService().Backoff().Remaining(string(provider.Claude), "selected")
	if remaining <= 0 || remaining > 45*time.Second {
		t.Fatalf("Remaining(selected) = %v, want (0s, 45s] from the manual 429", remaining)
	}
	if got := app.providerLifecycleService().Backoff().Remaining(string(provider.Claude), "other"); got != 0 {
