...
func TestProviderTurnActivity(t *testing.T) {
	...
	app.providerLifecycleService().NoteTurnActivity(string(provider.Claude))

	if !app.providerLifecycleService().TurnCompletedSince(string(provider.Claude), time.Time{}) {
		t.Fatalf("expected activity after a Claude turn completed (zero mark)")
	}
	if !app.providerLifecycleService().TurnCompletedSince(string(provider.Claude), before) {
		t.Fatalf("expected activity after a mark predating the turn")
	}
	if app.providerLifecycleService().TurnCompletedSince(string(provider.Codex), time.Time{}) {
		t.Fatalf("Claude activity must not read as Codex activity")
	}
	if app.providerLifecycleService().TurnCompletedSince(string(provider.Claude), time.Now()) {
...
func TestProbeClaudeRateLimits_UnmanagedProbe429RecordsAndEnforcesItsHold(t *testing.T) {
	...
	remaining := app.providerLifecycleService().Backoff().Remaining(string(provider.Claude), "")
	...
	if got := app.providerLifecycleService().Backoff().Remaining(string(provider.Claude), "some-account"); got != 0 {
