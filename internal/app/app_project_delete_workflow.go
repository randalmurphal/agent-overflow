...
// branch (`internal/workflowapp/discard_apply.go`), because there the human has said
...
func (a *App) ProjectDeletionPreview(projectID string) (ProjectDeletionPreview, error) {
	...
	footprint, err := a.projectApplication().WorkflowFootprint(projectID)
	if err != nil {
		return ProjectDeletionPreview{}, err
	}
	preview := ProjectDeletionPreview{
		ProjectID:       projectID,
		RunCount:        footprint.RunCount(),
		LiveRunIDs:      make([]string, 0),
		AutomationCount: footprint.AutomationCount(),
...
func (a *App) projectWorkflowCheckouts(
	footprint projectapp.WorkflowFootprint, projectPath string,
...
func (a *App) describeCleanupWorktree(
	...
	branch, registered := workflowapp.RegisteredWorktreeBranch(registry, target)
	described := ProjectCleanupWorktree{Path: target.Path, Branch: branch}
	if !workflowapp.WorktreeDirectoryPresent(target.Path) {
		return described
	}
	if !registry.Present {
