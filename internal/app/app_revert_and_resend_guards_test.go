...
func TestRevertAndResendValidatesArgs(t *testing.T) {
	...
	if err := revertAndResendForTest(app, context.Background(), "t1", "  ", RevertAndResendOptions{Content: "edited"}); err == nil ||
	...
	if err := revertAndResendForTest(app, context.Background(), "t1", "user:1", RevertAndResendOptions{Content: "  \n\t "}); err == nil ||
...
func assertRevertAndResendRejected(
	...
	rejection := revertAndResendForTest(app, context.Background(), threadID, userItemID, RevertAndResendOptions{
