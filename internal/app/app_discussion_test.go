...
func TestResolveDiscussionDefinitionFallsBackToGlobalWhenProjectDefinitionMissing(t *testing.T) {
	...
	def, err := app.discussionService().ResolveDefinition(thread, "Architects")
	if err != nil {
...
func TestResolveDiscussionDefinitionDoesNotHideProjectDefinitionErrors(t *testing.T) {
	...
	_, err = app.discussionService().ResolveDefinition(thread, "Architects")
	if err == nil {
...
func TestStartDiscussionCreatesChannelChildrenAndParticipantSessions(t *testing.T) {
	...
	if _, ok := app.deliberation(channel.ID); !ok {
...
func TestStartDiscussionCleansUpOnParticipantSessionFailure(t *testing.T) {
	...
	if got := app.sessionManager().runtime.ThreadSystemPromptCount(); got != 0 {
		t.Fatalf("thread system prompt count = %d, want empty after cleanup", got)
	}
	if got := app.discussionService().RuntimeCount(); got != 0 {
		t.Fatalf("deliberation count = %d, want empty after cleanup", got)
	}
}
...
func TestPostChannelMessageAndGetChannelMessages(t *testing.T) {
	...
	channel, err := discussion.NewChannelService(app.store).Create(thread.ID, "deliberation", 5)
	if err != nil {
...
func TestDeleteThreadRemovesDiscussionChildrenAndRuntimeState(t *testing.T) {
	...
	for _, child := range children {
		app.sessionManager().put(child.ID, session{Provider: child.Provider})
	}
	...
	if got := app.sessionManager().runtime.ThreadSystemPromptCount(); got != 0 {
		t.Fatalf("thread system prompt count = %d, want empty after delete", got)
	}

	if len(app.sessionManager().runtime.Snapshot()) != 0 {
		t.Fatalf("sessions = %v, want empty after delete", app.sessionManager().runtime.Snapshot())
	}
	if got := app.discussionService().RuntimeCount(); got != 0 {
		t.Fatalf("deliberation count = %d, want empty after delete", got)
	}
}
...
func TestSessionEventHandlerMirrorsDiscussionTurnsIntoChannelAndConcludes(t *testing.T) {
	...
	deliberation, _ := app.deliberation(parent.DiscussionID)
...
func TestDeliberationForChannelRebuildsFromStoreAfterRestart(t *testing.T) {
	...
	post := func(fromType, fromID, fromRole, content string) {
		t.Helper()
		if _, err := discussion.NewChannelService(app.store).PostMessage(discussion.PostMessageInput{
...
func TestPostChannelMessageSelfHealsWedgedConcludedDeliberation(t *testing.T) {
	...
	app.discussionService().InstallRuntime(channelID, discussion.RestoreDeliberation(
