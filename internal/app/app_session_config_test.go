...
func seedConfigReconcileThread(t *testing.T, app *App, id string, liveness *sessionLiveness) (string, chan string) {
	...
	app.sessionManager().put(id, session{Provider: thread.Provider, Token: "cfg-token", Liveness: liveness})
...
func TestThreadConfigBusySignals(t *testing.T) {
	...
	app.sessionManager().runtime.SetConfigReconnectQuietOverride(time.Hour)
	liveness := newSessionLiveness(time.Now())
	app.sessionManager().put(thread.ID, session{Provider: thread.Provider, Token: "busy-token", Liveness: liveness})
	if !app.threadConfigBusy(thread.ID) {
...
`
	...
	got := entry.LaunchOptions.ReasoningEffort
	...
	pending := app.sessionManager().runtime.PendingConfigReconnect(thread.ID)
	if pending {
