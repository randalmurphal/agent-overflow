...
// live session's launch options (session.LaunchOptions) against the thread
...
func (a *App) configReconnectPollInterval() time.Duration {
	poll, _ := a.sessionManager().runtime.ConfigReconnectOverrides()
	if poll > 0 {
		return poll
	}
...
func (a *App) configReconnectQuietWindow() time.Duration {
	_, quiet := a.sessionManager().runtime.ConfigReconnectOverrides()
	if quiet > 0 {
		return quiet
	}
...
// read-modify-write over session.LaunchOptions, and two concurrent reconciles
...
func (a *App) liveApplySessionConfig(threadID string) bool {
	...
	case sess.Claude != nil:
		...
		applied, err := sess.Claude.ApplyLiveUpdate(a.lifeCtx(), update, func(receipt claude.LiveApplyReceipt) {
			a.registerClaudeLiveConfigApplies(threadID, sess.Token, sess.LaunchOptions, update, receipt)
		...
		return true
	case sess.Codex != nil:
		update, ok := codex.PlanLiveUpdate(sess.LaunchOptions, opts)
		if !ok {
			return false
		}
...
func (a *App) scheduleLiveClaudeReconcile() {
	if !a.sessionManager().runtime.BeginLiveClaudeReconcile() {
		return
	}
...
func (a *App) runLiveClaudeReconcileSweeps() {
	for {
		...
		if a.sessionManager().runtime.FinishLiveClaudeReconcileSweep(a.shuttingDown.Load()) {
			continue
		}
		return
	}
...
func (a *App) reconcileSessionConfigStep(threadID string) {
	step := a.sessionManager().runtime.ReconcileSessionConfigStep()
	if step != nil {
		step(threadID)
		return
	}
...
func (a *App) schedulePendingConfigReconnect(threadID string) {
	if !a.sessionManager().runtime.BeginPendingConfigReconnect(threadID) {
		return
	}
...
func (a *App) clearPendingConfigReconnect(threadID string) {
	a.sessionManager().runtime.ClearPendingConfigReconnect(threadID)
}
