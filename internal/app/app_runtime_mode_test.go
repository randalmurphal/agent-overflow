...
func TestUpdateThreadRuntimeModeKeepsModeOnRestartFailure(t *testing.T) {
	...
	app.sessionManager().put(id, session{Token: "t"})
...
func TestSendRuntimeModeChangeReconnectsAfterInflightStart(t *testing.T) {
	...
	app.startSessionFn = func(threadID string) error {
		...
		case 1:
			...
			<-releaseFirstStart
			app.sessionManager().put(threadID, session{Provider: string(provider.Codex), Token: "old-mode-session"})
			return nil
		case 2:
			stored, err := app.store.GetThread(id)
			if err != nil {
			...
			app.sessionManager().put(threadID, session{Provider: string(provider.Codex), Token: "new-mode-session"})
			close(secondStart)
			return nil
		default:
	...
	final, _ := app.sessionManager().get(id)
