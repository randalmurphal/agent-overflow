...
func writeSlotCredentialDirectly(t *testing.T, app *App, accountID string, data []byte) {
	t.Helper()
	path, err := providerCredentialsForTest(t, app).AccountCredentialPath(string(provider.Claude), accountID)
	if err != nil {
		t.Fatal(err)
	}
...
func TestInactiveExpiredClaudeUsageRefreshSpendsNothing(t *testing.T) {
	...
	if remaining := app.providerLifecycleService().Backoff().Remaining(string(provider.Claude), "inactive"); remaining != 0 {
...
func TestInactiveLiveClaudeUsageRefreshProbesOverHTTP(t *testing.T) {
	...
	saved, err := providerCredentialsForTest(t, app).ReadCredential(string(provider.Claude), "inactive", false)
	if err != nil {
		t.Fatal(err)
	}
...
func TestInactiveClaudeUsageRefreshRecordsItsOwn429Hold(t *testing.T) {
	...
	remaining := app.providerLifecycleService().Backoff().Remaining(string(provider.Claude), "inactive")
	if remaining <= 0 || remaining > 45*time.Second {
		t.Fatalf("Remaining(inactive) = %v, want (0s, 45s] from the 429", remaining)
	}
	if got := app.providerLifecycleService().Backoff().Remaining(string(provider.Claude), "selected"); got != 0 {
