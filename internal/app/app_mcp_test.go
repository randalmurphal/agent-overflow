...
func newMCPTestApp(t *testing.T) (*App, string, string) {
	...
	deps := app.mcpDeps()
	deps.ClaudeConfig = claudeconfig.New(claudePath)
	deps.CodexConfig = codexconfig.New(codexPath)
...
`
	...
	projected, err := app.mcpService().CodexSessionMCPRows(context.Background(), sess)
	if err != nil {
		t.Fatalf("codexSessionMCPRows: %v", err)
	}
	rows := threadMCPServers(projected)
...
func TestListWorkspaceMcpServers_StaleStatusOverlayFlagged(t *testing.T) {
	...
	writeClaudeConfig(t, claudePath, `{"mcpServers": {}, "projects": {}}`)
	now := time.Now()
	claudeStore, err := app.mcpService().ClaudeConfig()
	if err != nil {
		t.Fatalf("ClaudeConfig: %v", err)
	}
	codexStore, err := app.mcpService().CodexConfig()
	if err != nil {
		t.Fatalf("CodexConfig: %v", err)
	}
	deps := app.mcpDeps()
	deps.ClaudeConfig = claudeStore
	deps.CodexConfig = codexStore
	deps.StatusCache = mcpstatus.NewWith(30*time.Second, nil, func() time.Time { return now })
	app.mcpApp = mcpapp.New(deps)
	app.mcpStatus().Put(mcpstatus.ServerStatus{
