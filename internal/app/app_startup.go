...
func (a *App) initStores() (string, *store.Store, error) {
	...
	providerCredentials, err := newCredentials(userHome, providerCredentialPolicy())
	if err != nil {
		closeErr := st.Close()
		return "", nil, errors.Join(
	...
	if err := a.ensureProviderAccountManager().Attach(
		accountStore,
		providerCredentials,
		filepath.Join(dbDir, "account-audit.log"),
	); err != nil {
		closeErr := st.Close()
		return "", nil, errors.Join(
			fmt.Errorf("attach provider account stores: %w", err),
	...
	if err := a.providerAccounts.ReconcileProviderHome(dbDir, userHome); err != nil {
		closeErr := st.Close()
		return "", nil, errors.Join(
			err,
			errorsx.WrapLifecycle("close store after provider account reconciliation failure", closeErr),
		)
	}
	...
	a.providerLifecycleService().LoadBackoff(filepath.Join(dbDir, "usage-backoff.json"))
...
func (a *App) initSubsystems(dbDir string, st *store.Store) error {
	...
	a.browser.manager = appbrowser.NewManager(
		...
		appbrowser.ManagerOptions{FileStateKey: a.fileKeychainOverride, PaneHost: a.paneHostOptions(), NativeWindow: a.browser.nativeWindow},
	)
