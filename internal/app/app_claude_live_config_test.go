...
func seedClaudeLiveConfigThread(t *testing.T, app *App, id, token string, launchOpts provider.SessionOptions) chan string {
	...
	app.sessionManager().put(id, session{Provider: thread.Provider, Token: token, LaunchOptions: launchOpts})
...
func launchOptsForThread(t *testing.T, app *App, threadID string) provider.SessionOptions {
	t.Helper()
	sess, ok := app.sessionManager().get(threadID)
	if !ok {
		t.Fatalf("no session registered for %s", threadID)
	}
	return sess.LaunchOptions
}
...
func TestPurgeClaudeLiveConfigStateIsTokenScoped(t *testing.T) {
	...
	app.sessionManager().put("purged", session{Token: "tok-a"})
	app.sessionManager().take("purged")
...
func TestRegisterClaudeLiveConfigAppliesEvictsStaleEntries(t *testing.T) {
	...
	app.sessionManager().runtime.RegisterClaudeLiveApplies(map[string]claudeLiveConfigApply{
		"cmd-old": {
			ThreadID: "t1", SessionToken: "tok-a", Axis: claudeLiveApplyAxisEffort,
			SentAt: time.Now().Add(-claudeLiveApplyStaleAfter - time.Minute),
		},
	}, time.Now().Add(-claudeLiveApplyStaleAfter-time.Minute), claudeLiveApplyStaleAfter)
...
func TestResolveSkipsSessionScopedEffectsWhenSessionGone(t *testing.T) {
	...
	pending := claudeLiveConfigApply{
		...
		Requested:      "xhigh",
		PreviousEffort: provider.EffortHigh,
	}
