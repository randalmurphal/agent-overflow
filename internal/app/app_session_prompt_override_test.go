...
func TestSpawnSurfacesAMemoryDirectoryFailureWithoutFailingTheSpawn(t *testing.T) {
	...
	_, live := app.sessionManager().get(threadID)
	if !live {
...
func TestPromptOverrideChangeDoesNotDisturbALiveSession(t *testing.T) {
	...
	app.sessionManager().runtime.SetConfigReconnectPollOverride(10 * time.Millisecond)
	app.sessionManager().runtime.SetConfigReconnectQuietOverride(time.Nanosecond)
	...
	live, _ := app.sessionManager().get(id)
	pending := app.sessionManager().runtime.PendingConfigReconnect(id)

	if pending {
		t.Fatal("a settings override edit queued a deferred restart on a live session")
	}
	if live.LaunchOptions.ReasoningEffort != provider.EffortLow {
		t.Fatalf("session effort = %q, want low (the unrelated change must still live-apply)", live.LaunchOptions.ReasoningEffort)
	}
	if live.LaunchOptions.SystemPrompt != "launched prompt" {
		t.Fatalf("session SystemPrompt = %q, want the prompt it launched with", live.LaunchOptions.SystemPrompt)
	}
	if !slices.Equal(live.LaunchOptions.DisabledTools, []string{"web_search"}) {
		t.Fatalf("session DisabledTools = %v, want the list it launched with", live.LaunchOptions.DisabledTools)
	}
