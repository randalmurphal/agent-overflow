...
// path of maybeGenerateThreadTitleWithAttachments through threadtitleapp. The
// thread title advances from the default to
...
func TestGeneratedThreadTitle_CodexPathHappy(t *testing.T) {
	...
	_, imagePath, ok, err := app.attachments.Get(record.ID)
	if err != nil || !ok {
		t.Fatalf("Get() = ok %v err %v", ok, err)
	}
	got, err := app.generateThreadTitlePrompt(
		thread,
		threadtitle.BuildPrompt("Fix title generation for Codex threads.", []store.Attachment{record}),
		[]string{imagePath},
	)
	if err != nil {
		t.Fatalf("generateThreadTitlePrompt() error = %v", err)
	}
	...
	generatedImagePath := nextArgAfter(gotSpec.Args, "--image")
	if generatedImagePath == "" {
		t.Fatalf("codex args missing --image: %v", gotSpec.Args)
	}
	if !filepath.IsAbs(generatedImagePath) {
		t.Fatalf("image path = %q, want absolute path", generatedImagePath)
	}
...
func TestGeneratedThreadTitle_RoutesToClaudeWhenConfigured(t *testing.T) {
	...
	got, err := app.generateThreadTitlePrompt(thread, threadtitle.BuildPrompt("Fix title generation.", nil), nil)
	if err != nil {
		t.Fatalf("generateThreadTitlePrompt() error = %v", err)
	}
...
// ---- Layer 2 (runtime retry) tests for generateThreadTitlePrompt ----
...
func TestGeneratedThreadTitle_Layer2PrimaryFailsAlternateSucceeds(t *testing.T) {
	...
	got, err := app.generateThreadTitlePrompt(thread, threadtitle.BuildPrompt("Fix title gen.", nil), nil)
	if err != nil {
		t.Fatalf("generateThreadTitlePrompt: %v", err)
	}
...
func TestGeneratedThreadTitle_Layer2BothFailReturnsPrimaryError(t *testing.T) {
	...
	_, err := app.generateThreadTitlePrompt(thread, threadtitle.BuildPrompt("anything", nil), nil)
	if err == nil {
