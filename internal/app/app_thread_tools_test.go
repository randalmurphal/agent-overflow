...
func TestThreadToolsCannotReadOrDiscoverPreparingFork(t *testing.T) {
	...
	for name, read := range map[string]func() error{
		"thread": func() error { _, err := f.adapter.Thread(t.Context(), pending.ID); return err },
		"window": func() error {
			_, err := f.adapter.ResolveWindow(t.Context(), threadtools.WindowQuery{ThreadID: pending.ID, Kind: threadtools.WindowAll})
			return err
		},
		"transcript": func() error {
			_, err := f.adapter.Transcript(t.Context(), threadtools.TranscriptQuery{ThreadID: pending.ID, Limit: 10})
			return err
		},
		"payload": func() error {
			_, err := f.adapter.ItemPayload(t.Context(), threadtools.PayloadQuery{ThreadID: pending.ID, ItemID: "shared"})
			return err
		},
		"export": func() error {
			_, err := f.adapter.ExportTranscript(t.Context(), threadtools.ExportQuery{ThreadID: pending.ID})
			return err
		},
	} {
		t.Run(name, func(t *testing.T) {
			err := read()
			if !errors.Is(err, store.ErrForkPreparing) {
				t.Errorf("read preparing fork: %v, want ErrForkPreparing", err)
			}
			if code, message, ok := errorsx.PublicDetails(err); !ok || code != threadtools.CodeInvalidRequest || !strings.Contains(message, "still being prepared") {
				t.Errorf("preparation refusal lacks actionable public prose: %q %q", code, message)
			}
		})
	}
	for _, query := range []string{"", "quokka", "pending"} {
		page, err := f.adapter.SearchThreads(t.Context(), threadtools.SearchQuery{Query: query, Limit: 10})
		if err != nil || len(page.Rows) != 0 {
			t.Errorf("search %q exposed preparing fork: %+v err=%v", query, page, err)
		}
	}
	for _, ref := range []string{pending.ID, "pending"} {
		resolved, err := f.adapter.ResolveThreadRef(t.Context(), ref)
		if err != nil || len(resolved.Matches) != 0 {
			t.Errorf("resolve %q exposed preparing fork: %+v err=%v", ref, resolved, err)
		}
	}
	if _, err := f.app.store.FinishForkPreparation(pending.ID); err != nil {
		t.Fatal(err)
	}
	if _, err := f.adapter.Thread(t.Context(), pending.ID); err != nil {
		t.Fatal(err)
	}
	page, err := f.adapter.SearchThreads(t.Context(), threadtools.SearchQuery{Query: "quokka", Limit: 10})
	if err != nil || len(page.Rows) != 1 || page.Rows[0].Thread.ID != pending.ID {
		t.Fatalf("ready fork not searchable: %+v err=%v", page, err)
	}
}
