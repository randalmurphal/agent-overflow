...
- `spinner.go` — `Sprite` / `Files` wire shapes and
  `Service{Files, EnsureBoot}` (boot under a private mutex, listing
  unlocked).
- `assets/SPINNERS.md` — the authoring reference. Embedded with
...
`CLAUDE.md` principle 1). This package pairs files, bounds them, and
hands them over.
