...
// Deps are the process-level capabilities the updater needs from its host.
// Keeping these as narrow functions lets the updater own its state machine
// without depending on package main or the transport implementation.
type Deps struct {
	Context              func() context.Context
	IsShuttingDown       func() bool
	Emit                 func(eventchan.Channel, any)
	Exit                 func(int)
	RestartWatchdogDelay time.Duration
}

// Service owns the complete in-app update lifecycle for one process.
// It must be used through a pointer: the state contains a mutex and live timers.
type Service struct {
...
// run before the RPC methods report support.
func New(version string, deps Deps) *Service {
	return &Service{deps: deps, version: version}
}

func (a *Service) context() context.Context {
...
func (a *Service) isShuttingDown() bool {
	return a.deps.IsShuttingDown != nil && a.deps.IsShuttingDown()
}

func (a *Service) emit(name eventchan.Channel, data any) {
	if a.deps.Emit != nil {
		a.deps.Emit(name, data)
	}
}

// In-app self-update surface, shared by both builds. The native adapter passes
// the Wails application updater to Configure; the WSL adapter asks ConfigureWSL
// to build a headless updater over the same providers and stage the artifact
// for the Windows launcher. Every method guards an unconfigured handle and
...
// BridgedEvents returns the Wails updater lifecycle events the desktop host
// must subscribe to. A new slice prevents callers from mutating the mapping.
func BridgedEvents() []string {
	events := make([]string, 0, len(updaterEventBridge))
	for event := range updaterEventBridge {
...
// updaterReadyChannel and updaterErrorChannel name the two channels the service
...
var (
	...
	// cannot self-update: an unconfigured host, including unstamped "dev"
	// builds. The frontend treats it as "hide the update UI".
...
func (a *Service) CheckForUpdate() (UpdateAvailability, error) {
	if a.updater.handle == nil {
		return UpdateAvailability{Supported: false, CurrentVersion: a.version}, nil
	}
...
func (a *Service) DownloadUpdate(tag string) error {
	if a.updater.handle == nil {
	...
	go func() {
		...
		if tag != "" {
			...
			rctx, rcancel := context.WithTimeout(a.context(), updaterCheckTimeout)
			a.updater.mu.Lock()
			if a.updater.provider != nil {
		...
		ctx, cancel := context.WithTimeout(a.context(), updaterDownloadTimeout)
		defer cancel()
...
// success event.
func (a *Service) updaterErrorEmitter(info updater.ErrorInfo) func() {
	return func() { a.emit(updaterErrorChannel, info) }
}

// defaultRestartExitWatchdogDelay preserves a safe fallback for standalone
// package use. The production App passes its root-owned shutdown budget through
// Deps.RestartWatchdogDelay so that cross-package policy is explicit.
const defaultRestartExitWatchdogDelay = 25 * time.Second
...
// returns a disarm function. Fires through Deps.Exit (os.Exit when nil) so
// tests can observe the trigger without dying.
func (a *Service) armRestartExitWatchdog(delay time.Duration) (disarm func()) {
