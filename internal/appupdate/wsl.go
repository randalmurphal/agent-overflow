// This file is the WSL half of in-app self-update: the process that
...
// ConfigureWSL before the transport server starts and never mutated after,
...
// WSLConfig contains every host-derived input needed by the WSL updater. The
// root boot package resolves environment and link-stamped globals before
...
func (a *Service) ConfigureWSL(config WSLConfig) error {
	if config.StagingRoot == "" {
		return errors.New("updater: WSL staging root is empty")
	}
	if config.MarkerDir == "" {
	...
	if config.BackstopTimeout <= 0 {
		config.BackstopTimeout = wslInstallBackstopTimeout
	}
	mode := &wslUpdateMode{
	...
	if err := a.Configure(u, providerConfig); err != nil {
		return err
	}
	a.updater.wsl = mode
...
func reconcileWSLUpdateMarker(a *Service, currentVersion string, mode *wslUpdateMode) {
...
func (a *Service) setUpdateApplyFailure(notice string) {
	a.updater.mu.Lock()
...
// after its notification transport has been wired.
func (a *Service) ApplyFailure() string {
	a.updater.mu.Lock()
	defer a.updater.mu.Unlock()
	return a.updater.applyFailure
}
...
func (a *Service) stageWSLUpdate(rel *updater.Release) (terminal func()) {
	mode := a.updater.wsl
	...
	// the one we are about to hand over. Clearing service state first keeps the two
...
func (a *Service) failWSLStaging(cause error) func() {
...
func (a *Service) ReportUpdateInstallStatus(stage, version, message string) error {
...
func (a *Service) armWSLInstallDeadlineLocked(after time.Duration, message string) {
...
func (a *Service) failWSLInstallOnDeadline(gen uint64, message string) {
	if a.isShuttingDown() {
...
func (a *Service) settleWSLInstallLocked() {
	if a.updater.installTimer != nil {
		a.updater.installTimer.Stop()
...
type wslUpdaterHost struct{ service *Service }
...
func (h wslUpdaterHost) Emit(name string, data ...any) bool {
	...
	h.service.emit(channel, payload)
	return true
}
