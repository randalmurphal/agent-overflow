...
// eventRecorder captures Service emit traffic the way the production transport bus
...
func newEventRecorder(a *Service) *eventRecorder {
	r := &eventRecorder{}
	a.deps.Emit = func(name eventchan.Channel, data any) {
		r.mu.Lock()
		ev := recordedEvent{channel: string(name), data: data}
...
func newWSLTestApp(t *testing.T, srv *httptest.Server, current string, deadlines wslTestDeadlines) (*Service, *eventRecorder, *wslUpdateMode) {
	t.Helper()
	mode := &wslUpdateMode{
	...
	a := &Service{updater: appUpdaterState{wsl: mode}, version: current}
...
func (a *Service) busySnapshot() bool {
	a.updater.mu.Lock()
	defer a.updater.mu.Unlock()
	return a.updater.busy
}

func TestConfigureWSLTargetsLauncherArtifact(t *testing.T) {
	...
	if a.updater.handle.CurrentVersion() != "0.0.10" {
		t.Fatalf("updater version = %q, want 0.0.10", a.updater.handle.CurrentVersion())
	}
	wantStaging := filepath.Join(stagingRoot, selfupdate.StagingDirName)
...
func TestConfigureWSLRequiresStorageRoots(t *testing.T) {
	a := New("0.0.10", Deps{})
	if err := a.ConfigureWSL(WSLConfig{MarkerDir: t.TempDir()}); err == nil {
		t.Fatal("ConfigureWSL accepted an empty staging root")
	}
	if err := a.ConfigureWSL(WSLConfig{StagingRoot: t.TempDir()}); err == nil {
		t.Fatal("ConfigureWSL accepted an empty marker directory")
	}
}
...
func TestWSLInstallFailedReportUnwinds(t *testing.T) {
	...
	// The artifact is genuinely still staged on the Windows side, so the service's
...
func handOffForTest(t *testing.T, a *Service, rec *eventRecorder) {
	t.Helper()
	stageForTest(t, a, rec)
	from := rec.mark()
	if err := a.RestartToUpdate(); err != nil {
...
func TestWSLInstallDeadlineStandsDownDuringShutdown(t *testing.T) {
	...
	a.deps.IsShuttingDown = shuttingDown.Load
	handOffForTest(t, a, rec)
	if err := a.ReportUpdateInstallStatus(selfupdate.StatusProceeding, "0.0.8", ""); err != nil {
		t.Fatalf("ReportUpdateInstallStatus(proceeding): %v", err)
	}
...
func stageForTest(t *testing.T, a *Service, rec *eventRecorder) {
	t.Helper()
...
func TestReportUpdateInstallStatusRejectsUnknownStage(t *testing.T) {
	a := &Service{updater: appUpdaterState{wsl: &wslUpdateMode{markerDir: t.TempDir(), stagingDir: t.TempDir()}}}
...
func TestReportUpdateInstallStatusUnsupportedOffWSL(t *testing.T) {
	a := &Service{} // desktop / test build: no WSL mode
...
func TestWSLUpdaterHostOnEventReturnsUsableRemover(t *testing.T) {
	...
	remove := wslUpdaterHost{service: New("dev", Deps{})}.OnEvent("anything", func(any) {})
	if remove == nil {
...
func TestWSLUpdaterHostOpenWindowIsNilSafe(t *testing.T) {
	if h := (wslUpdaterHost{service: New("dev", Deps{})}).OpenWindow(updater.WindowOptions{}); h != nil {
