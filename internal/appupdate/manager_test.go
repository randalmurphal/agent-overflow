...
func TestUpdaterRPCsUnsupportedWhenNil(t *testing.T) {
	a := New("dev", Deps{}) // no updater configured
	...
	if err := a.RestartToUpdate(nil); !errors.Is(err, ErrUpdatesUnsupported) {
...
func TestRestartExitWatchdogFiresAfterDelay(t *testing.T) {
	...
	a := New("dev", Deps{Exit: func(code int) { fired <- code }})
...
func TestRestartWithExitWatchdogDisarmsOnSpawnFailure(t *testing.T) {
	...
	// armed, Deps.Exit would eventually be called. A short observation
