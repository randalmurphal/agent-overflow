...
func newUsageLimitObserveHarness(t *testing.T, providerName string, identified bool) *usageLimitObserveHarness {
	...
	h := &usageLimitObserveHarness{
		store: dataStore, outcome: make(chan engine.Outcome, 1), now: time.Unix(1_700_000_000, 0),
	}
	h.runner = newTestRunner(t, nil, dataStore, staticWorkflowProfileSource{value: &profile.Profile{}})
	...
	if identified {
		h.attempt.dispatchIdentity = DispatchIdentity{
	...
	if err := dataStore.CreateWorkItem(store.WorkItem{
		ID: key.ItemID, ProjectID: project.ID, Goal: "usage limit", WorkflowID: "wf",
...
func TestUsageLimitRefusalParksImmediatelyWithoutRetries(t *testing.T) {
	for _, test := range []struct {
		...
		t.Run(test.provider, func(t *testing.T) {
			...
			scope, err := h.store.GetWorkflowProviderUsageScope(outcome.ProviderUsageScopeID)
			if err != nil {
				t.Fatal(err)
			}
			...
			if at, err := h.store.WorkItemAutoResumeAt("run-limit"); err != nil || at != 0 {
