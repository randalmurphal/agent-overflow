...
// tests that exercise the App's DownloadUpdate guards (which need a non-nil,
...
// mock GitHub server, so App-level updater RPCs can be driven end to end.
func newUpdaterApp(t *testing.T, srv *httptest.Server, current string) (*App, *targetableProvider) {
	t.Helper()
