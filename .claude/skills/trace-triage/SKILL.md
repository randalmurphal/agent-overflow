...
3. **Standard passes** over each window. Each pass ends ruled-in or ruled-out, with `seq` numbers as evidence:
   ...
   - **Glide cadence** (chunky, juddery or hitching motion): run `node scripts/uitrace/chase-ticks.mjs <bookmark>` first. It replays `scroll.spring.ticks` for every chase before the marker as step/frames pairs with target-change marks, and prints the chase's `droppedFrames`, `lateTicks`, `unevenWrites` and `stepJumps`. Holes with high lateness are main-thread work; holes with low lateness are presentation; uneven writes without holes are the sampler's pixel rhythm. Sampled `spring.tick` writes are not evidence of cadence.
...
5. **Verdict.** Report: symptom restated in observable terms → per-pass ruled-in/ruled-out with evidence → leading hypothesis → how to falsify it today (harness repro, browser test, targeted spike). INCONCLUSIVE is a legal verdict for a pass, but the overall answer is never "wait for it to happen again": if the trace cannot discriminate the surviving hypotheses, the deliverable becomes the always-on diagnostic addition that would, proposed concretely.
