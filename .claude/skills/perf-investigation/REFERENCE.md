...
- 2026-08-26 **165Hz frame-drop investigation (user's app, investigation only — no changes built).** User: slight, near-constant, barely perceptible frame dropping after an overnight session. Display is 165Hz → 6.06ms frame budget; any main-thread task >6ms drops a frame. Three attributed sources, no fix chosen yet:
  1. **Idle-trim GC metronome.** `idleMemoryTrim.ts` re-requests every 5min for as long as idleness persists; backend gates only on open-turn + 4min floor. launcher.log: 717 trims, every ~5min all night, "renderer GC done in 30-51ms" — and growing with session age (36→44→51ms). The 20s frames trace caught one as the sole 41.9ms task (MajorGC "low memory notification", reclaim ~1MB of a 31MB heap). Each hit = 5-8 dropped frames. Blind spot: `hasActiveProviderTurn` false does not mean not-animating — trims landed mid-glide. Lever: require activity since the last trim (turn completed / N MB allocated), and/or defer while a glide runs. Near-zero cost, kills ~150 pointless stalls/night.
  2. **Reveal-tick rAF frames at 12-15ms.** During streaming, ~27 tasks >8ms per 120s, all one shape: FireAnimationFrame ~7-9ms (svelte microtask flush 6-8.5ms inside) + Paint 2-3ms + layout/layerize ~1.5ms. = 2 dropped frames each, ~13/min. The flush is the reveal step's effect cascade (svelte-vendor #process/flush 10.1% inclusive).
  3. **Scroll-metric read-after-write thrash on glide ticks.** Style-recalc causes over 120s streaming: targetScrollTop (spring write) 1384 passes/28.9k el/274ms + readScrollMetrics (overlay scrollbar read) 1310 passes/27.5k el/242ms; cpu 60s: `get scrollTop` 141ms + `get scrollHeight` 96ms = 15.5% of all JS-visible busy. Writes dirty style, the same frame's reads force the recalc, ~22x/s. Lever: batch reads before writes per frame, or cache metrics per frame in the controller.
  - Clean baseline pinned: outside trims and streaming, 150s rAF capture = 24,749 frames, median 6.1ms, p99 6.2, worst gap 11.6ms, zero LoAF ≥50ms; 10-min stall watch: zero ≥50ms, ≥20 stalls of 10-24ms. New probes committed: `framedrops` (rAF-gap + LoAF), `mainstalls` (timer-drift, no forced frames). MS Teams webviews own the machine's other big WebView2 processes (1GB/74 CPU-min) — not us. ao-perfsample expired 04:00 and watched the previous instance; left stopped.
