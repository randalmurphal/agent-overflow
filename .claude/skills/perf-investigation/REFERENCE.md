...
- Codex rollout-tail pair (deferred 2026-08-25, user-approved, waiting on codex-package work settling): gate `session_rollout_notifications.go`'s tail on resume-with-observed-spawn relevance, and reuse the partial-line buffer (`append(partial[:0], ...)` + capacity shed) — the fresh copy per 150ms tick was ~28MB/h. Upstream PR adding `experimentalRawEvents` to `thread/resume` is the clean kill.
