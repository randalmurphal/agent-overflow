...
- 56 Documents / 55 Frames in the DOM counters with one real frame (2026-08-25): SVGImage documents. Blink gives every DISTINCT SVG-as-image resource its own internal page+frame+document, and the mask-icon patch's data URIs are 47 distinct resources (581 mounted icons share them). Bounded by icon vocabulary, static, does not scale with mounts. Candidate if baseline blink_gc needs cutting: consolidate icons into one external SVG addressed by fragment — needs a spike to confirm Blink shares one resource document across fragments; estimated ceiling 5-15MB. Not built.
