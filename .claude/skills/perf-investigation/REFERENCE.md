...
- 2026-08-25 `f3552fb8`: the ambient pulse dots ran per-element `steps()` opacity CSS animations, and any active opacity animation promotes its element to a composited layer (kActiveOpacityAnimation) — 11 standing layers for 6px dots. The ticker now writes three root custom properties (`--ambient-pulse-o/o2/o4`) and dots use `opacity: var(...)`: soak-verified 11→0 pulse CSSAnimations, LED-only promotions remain, present-rate policy unchanged. Third flip on this feature; the doctrine history lives in `app.css` and `ambientTicker.ts` headers.
...
- 2026-08-25 `c776ab8c`: idle-triggered memory-reducing GC. 3min input-idle in the frontend → `RequestWebviewMemoryTrim` (LocalOnly; skipped during active provider turns, 4min floor) → ephemeral loopback-only `webview:trim` directive → the Windows launcher fires `HeapProfiler.collectGarbage` via the fork's `CallDevToolsProtocol` (branch `ao-beta-memory-trim`, 2d9e0221958f). Returns the no-decommit ratchet's committed pages (~54MB measured on a 293MB/20MB-live renderer). Windows/WSL reach only — native desktop and `--connect` emit into silence. This is the idle lever; active-use decrease stays open below.
...
- Active-use memory decrease (the user's stated primary goal): the idle trim only fires between turns. Candidates to investigate: Chromium memory-pressure signals mid-activity, a reducing GC scheduled into turn gaps, and the freeze-purge sawtooth-low (~28.8MB) as a while-visible equivalent. Perf-cost A/B belongs on the soak rig.
