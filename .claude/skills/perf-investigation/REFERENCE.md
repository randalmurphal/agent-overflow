...
- 2026-08-26 **165Hz frame-drop campaign — VERIFIED LIVE (post-restart 1h framedrops capture, build 91879d7f, 17:17-18:17).** 593,708 frames, median 6.1 / p99 6.2 identical to baseline. Verification recipe that closed it: `t − at ≈ 77.6s` (LoAF page-time vs capture-time offset, anchored by the one event present in both lists) joins gaps↔LoAFs, and launcher.log `GC done` lines date every trim. The three attributed sources and their outcomes:
  ...
  - Post-fix capture residuals: the 75ms streaming `handleResizeEntries` LoAF DID NOT RECUR (the only RO-heavy LoAF was a real pane drag-drop, 53.7ms total = ondrop 27.9 + RO 20.5 — modestly under the pre-fix 55-97ms, consistent with the RO half shrinking; drop handlers still the open item). One NEW unattributed: a 114.8ms script-less LoAF at 18:04:37 mid-active-window, 2min from the nearest trim, no script ≥5ms — single occurrence; candidates are a major V8/Oilpan GC under load or system contention. Watch for recurrence before instrumenting.
...
- 2026-08-26 **residual-stutter sweep, remaining open items** (1h live capture + clone correlation):
  ...
  - **3-pane beat collision (decision-sheet candidate, not built):** with N panes streaming, all N beats land in one rAF flush (~5ms each; worst measured 27.8ms before / 25.1 after). Round-robining pane commits across frames (≤2-frame commit latency, reveal drain pacing untouched) would cap a collision frame near one pane's beat. Scheduling-only, but it alters cross-pane commit timing — user call.
  - **ComposerToolbar density measurer (small, open):** a MutationObserver on its own subtree (childList+characterData, i.e. the toolbar's token/limit text) re-measures per streaming beat, and `measureComposerToolbarCompact` does a write→read→write double forced pass when compact (toggles `data-compact` around a scrollWidth read). ~0.7ms self + 2 rAF callbacks in a worst frame, 19-21 forced passes per storm run. Fix shape: measure only on RO size change + explicit content-change signals, and avoid the mid-frame attribute toggle.
  ...
  - Two mid-turn 36ms gaps sat on the trim's 5s grid but with a turn open (backend refuses trims there) — unattributed; candidates are streaming-heap major GC or WSL subprocess CPU contention. Watch for recurrence post-restart; the git-fetch/liveness contention theory found NO fetch bursts in a 6.7min spawn watch (2 status spawns only, 1 gitwatch watcher live).
