...
- **Non-composited pane scroller — THE per-frame glide cost and the streaming-load memory refill (2026-08-25, measured fix candidate on the decision sheet, not committed).** The pane timeline scroller has no composited scrolling layer, so EVERY scroll offset change — the spring's per-frame `scrollTop` write AND real wheel input alike — runs a full main-frame lifecycle whose Layerize pass costs ~1.1-1.6ms (155/s during two-pane glide, 17-25% of main thread). The write was never the mechanism: wheel-gesture scrolling with the springs parked costs the same (Layerize 2.7s/12s; `probe scrollgesture`). Elimination chain, all on the soak under two-pane streaming glide: reduced-motion (glide off) dropped private refill 171.4→18.5MB/30s and Oilpan garbage 108→1MB/30s; `animation: none` left allocation flat; scrollbars hidden cut Paint 4203→983/12s with Layerize unchanged; scrollbar objects removed (`scrollbar-width: none`), Layerize still 1841/12s. Holding `will-change: scroll-position` on `[data-testid="message-timeline-scroll"]` (csshold): same glide, Layerize 1922ms/1857x → 170ms/1860x (count unchanged — Blink takes the direct scroll-offset-transform path instead of a full `PaintArtifactCompositor::Update`), main busy 340→168ms/s, wheel-scroll main busy 510→212ms/s with scroll dispatch coalesced 136→57/s, Oilpan class churn 5.15→2.58MB/min with the whole geometry-mapper family gone from the census (PlaneRootTransform 1.22, ClipCacheEntry 0.74, PendingLayer 0.41, ScopedForcedUpdate 0.24, ComputedStyle 0.25MB/min — all absent under the hold), blink_gc committed growth +35→+10MB/30s. Costs, measured: each visible pane gains a full-scrollback scrolling-contents layer, NOW-bin rasterized ≈ 9-14MB tiles/pane (+~28MB renderer `cc/tile_memory` for two panes, mirrored in GPU shared images); a fast fling over unrastered scrollback can checkerboard where main-thread scrolling could not; `architecture.test.ts`'s app-wide will-change ban (written against TRANSFORM promotion — a second paint position outside the scrollTop chokepoint) would need a deliberate, user-approved carve-out for `scroll-position`, which promotes the chokepoint's own mechanism rather than bypassing it.
...
- `handleScroll` (intent.ts) shows as a forced style recalc per scroll event (1636 passes/12s while wheel-scrolling): it pays whatever style dirt is PENDING (ambient-animation ticks, ~12 el/pass) per the a56ca00a forced-recalc-reader rule. Falls away as scroll dispatch coalesces under composited scrolling; not its own lever.
