...
- cc/tile_memory scales with composited planes on screen, about 6-10MB per visible pane at this window size. It is raster output, not app state.
- blink_gc (Oilpan) grows to a plateau over the first hour: committed pages stay committed after a normal GC (113MB committed vs 15MB allocated seen at 1.5h). Only a memory-reducing GC (page hidden, memory pressure) returns them. Committed ≫ allocated with a flat allocated line is fragmentation, not a leak.
...
- A `PlaneRootTransform` row growing in the blink class breakdown means non-2D transform paint nodes are being regenerated per frame: any transform or scroll change clears `GeometryMapperTransformCache`, and each such node re-allocates 288B on the next paint.
...
- Glide residue `rotate: 0.0001deg` in `chokepoint.ts`: deliberate, defeats compositor pixel snapping. Leave it.
