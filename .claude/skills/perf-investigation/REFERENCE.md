...
- 2026-08-26 **sidebar running-tier order churn — DECISION SHEET item, not silently fixable.** Concurrently-streaming threads in one project keep swapping rank inside the running tier (activity is sort input; interleaved beats alternate which thread is "most recent"), and every real reorder runs svelte's FLIP measure (gBCR over the animated rows) + `element.animate` per moved row — ~50ms per storm run landing in burst frames beside heavy flushes (G capture: `apply`/`Ki` 45ms incl in the ≥16ms windows). The no-op-reconcile half is already fixed (`a84ac3de` identity cutoffs); what remains is REAL reorders. Any fix changes visible ordering behavior (freeze relative order among running threads, or quantize activity) — user's call. Measured ceiling: the FLIP slice of streaming burst frames only.
