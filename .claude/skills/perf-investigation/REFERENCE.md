...
- 2026-08-25 `f3552fb8` + `da78203a`: the ambient pulse dots ran per-element `steps()` opacity CSS animations, and any active opacity animation promotes its element to a composited layer (kActiveOpacityAnimation) — 11 standing layers for 6px dots. First fix wrote three root custom properties, which REGRESSED to 2fps springs on the live app: a root custom-property write invalidates style for the whole document (381 passes × ~3,500 el × 22-30ms per 10s, ~195ms/s — soak missed it, small document + wrong metric). Final form: the ticker writes each dot's opacity INLINE (glow-style mark-and-sweep, s2/s4 as stagger markers), one 6px span per invalidation, still zero Animation objects. Fourth flip; doctrine history in `app.css` and `ambientTicker.ts` headers. RULE: never write an animated custom property on the document root — root writes are for one-shot, rare values only (theme rewrite goes through a `<style>` element for the same reason).
