...
- 2026-08-26 **residual-stutter sweep, remaining open items** (1h live capture + clone correlation):
  ...
  - **Virtualizer resize storm, OPEN — next concentrated target.** `handleResizeEntries` (TimelineVirtualizer) hit 75.8ms in one live LoAF during ordinary streaming (preceded by a 6.8ms WS message in the same 96ms frame) and 20-30ms inside every pane drag-drop frame. The handler itself is lean; the cost is downstream of `engine.applyMeasurements` (projection + synchronous re-render inside the RO callback). Reproduce with real content: `ao-harness clone` → seed/open real threads → `scenario from-thread --set` → replay with `perf start` + CDP profile (single-pane headless replay reached 24.6ms open-burst and 19.9ms mid-drain ticks, dominated by GC + gBCR; the 75ms shape likely needs multi-pane + real viewport — harness-wsl leg).
  - **Pane drag-drop frame 55-79ms** (`PaneHost` `onHostDrop`/`onPaneDrop` ~26-32ms + the RO storm ~20-30ms in the same frame): one-shot gesture cost, observed three times live. Same downstream as the resize storm; fixing that likely halves this too.
