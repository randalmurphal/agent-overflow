...
- Continuous-fleet trim gap: with turns almost always open (heavy multi-thread use), zero-turn windows are rare and the between-turns trim rarely fires. The candidate is gating on "visible pane streaming" instead of "any turn" (frontend knows visibility; a 58ms stall while only background threads stream delays buffered writes nobody is watching; regrowth after a mid-stream GC measured ~4-6MB/25s on soak). On the decision sheet — it schedules stalls while agents work in background.
