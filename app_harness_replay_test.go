...
// an enabled event-log manager — the pieces the record/replay bundle
// flow touches. No transport bus: emissions land in testEmitHook.
func newHarnessTestApp(t *testing.T) (*Harness, *App) {
	t.Helper()
	root := t.TempDir()
	...
	t.Cleanup(func() { mgr.SetEnabled(false) })
...
func logHarnessEvent(t *testing.T, app *App, ts time.Time, threadID, kind string, data any) {
	t.Helper()
...
func TestHarnessRecordAndReplayBundleRoundTrip(t *testing.T) {
	...
	logHarnessEvent(t, app, base, threadID, "provider:item_event", map[string]any{"n": 0})
	...
	logHarnessEvent(t, app, base.Add(10*time.Millisecond), threadID, "provider:item_event", map[string]any{"n": 1})
	logHarnessEvent(t, app, base.Add(20*time.Millisecond), threadID, "provider:turn_completed", map[string]any{"n": 2})
	...
	bundleDir := filepath.Join(h.paths.DataDir, "bundles", "flicker-repro")
...
func TestHarnessRecordStopWithNoEvents(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(h.paths.DataDir, "bundles", "empty")); !os.IsNotExist(err) {
...
func TestHarnessRecordStartValidation(t *testing.T) {
	...
	logHarnessEvent(t, app, time.Now(), "t1", "provider:item_event", map[string]any{"n": 1})
	if _, err := h.HarnessRecordStop(); err != nil {
		t.Fatalf("HarnessRecordStop: %v", err)
	}
	if _, err := h.HarnessRecordStart("dup", "t1"); err == nil {
...
// seedHarnessThread inserts the minimal project + thread rows the
// turns table's FK chain requires.
func seedHarnessThread(t *testing.T, app *App, threadID string) {
	t.Helper()
...
func TestHarnessRecordStartRefusesActiveTurn(t *testing.T) {
	h, app := newHarnessTestApp(t)
	seedHarnessThread(t, app, "thread-live")
	...
	if err := app.store.UpdateTurnCompleted("thread-live:1", time.Now().UnixMilli(), "end_turn", "", "", ""); err != nil {
...
func TestHarnessRecordStopDetectsRotation(t *testing.T) {
	...
	logHarnessEvent(t, app, time.Now(), threadID, "provider:item_event", map[string]any{"n": 0})
	if _, err := h.HarnessRecordStart("rotated", threadID); err != nil {
		t.Fatalf("HarnessRecordStart: %v", err)
	}
	logHarnessEvent(t, app, time.Now(), threadID, "provider:item_event", map[string]any{"n": 1})
...
func TestHarnessResetClearsHarnessState(t *testing.T) {
	...
	workspaces := filepath.Join(h.paths.DataRoot, "workspaces", "proj")
	...
	if _, err := os.Stat(filepath.Join(h.paths.DataDir, "bundles", "doomed")); !os.IsNotExist(err) {
		t.Fatalf("in-flight recording dir survived reset (stat err %v)", err)
	}
	if _, err := os.Stat(filepath.Join(h.paths.DataRoot, "workspaces")); !os.IsNotExist(err) {
...
func TestHarnessReplayBundleRefusesActiveReplay(t *testing.T) {
	...
	seedHarnessThread(t, app, "post-snapshot-thread")

	// Park a paused replay, then try to restore the bundle over it.
	events := filepath.Join(h.paths.DataDir, "bundles", "guard", "events.jsonl")
	...
	if _, err := app.store.GetThread("post-snapshot-thread"); err != nil {
...
func TestHarnessReplayBundleRefusesCorruptEvents(t *testing.T) {
	...
	events := filepath.Join(h.paths.DataDir, "bundles", "corrupt", "events.jsonl")
	if err := os.WriteFile(events, []byte("{not json}\n"), 0o600); err != nil {
		t.Fatalf("corrupt events file: %v", err)
	}

	// Written after the snapshot: a restore would erase it.
	seedHarnessThread(t, app, "survives-corrupt-bundle")
	...
	if _, err := app.store.GetThread("survives-corrupt-bundle"); err != nil {
...
func TestHarnessEmitValidation(t *testing.T) {
	h, app := newHarnessTestApp(t)
	var got string
	app.testEmitHook = func(name string, data any) { got = name }
