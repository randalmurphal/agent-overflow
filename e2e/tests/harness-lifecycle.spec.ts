...
test('reaps a child that prints malformed bootstrap JSON', async () => {
  ...
  `);
  try {
    await expect(launchHarness({ binary: fake.binary, dataDir: path.join(fake.root, 'data'), timeoutMs: 200, env: { AO_PID_FILE: fake.pidFile } })).rejects.toThrow('unparseable harness bootstrap');
