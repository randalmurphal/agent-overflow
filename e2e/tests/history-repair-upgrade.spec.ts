...
// its data, the watermark advances to v119 with no failure recorded, and the
// repaired thread renders its rows.
...
test('an upgrade from v118 repairs stored history in the background', async ({ page }) => {
  ...
  try {
    ...
    await expect.poll(() => watermark(dbPath), { timeout: 60_000 }).toBe(119);
