...
/** Spawn the backend with a private process group and a bounded fallback VM. */
...
export async function captureProcessIdentity(pid: number | undefined): Promise<ProcessIdentity> {
  ...
  // Unix containment starts through `/bin/sh` so it can apply ulimit before
  // exec'ing the backend. Do not authenticate that short-lived wrapper. A
  // later identity check would otherwise reject the same PID when exec turns
  // it into the backend process.
  for (let attempt = 0; attempt < 100; attempt += 1) {
    const identity = await captureProcessIdentityOnce(pid);
    if (!isShellExecutable(identity.executable) || attempt === 99) return identity;
...
export async function verifyProcessGroupMemberProof(
  ...
  try {
    ...
    return (
      current.birth === proof.birth &&
      path.resolve(current.executable) === path.resolve(proof.executable) &&
      current.groupId === proof.groupId
    );
  } catch {
    return false;
  }
